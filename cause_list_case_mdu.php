...
$case_num=$_POST['case_no'];
$case_year=$_POST['case_year'];

$securimage = new Securimage(array('namespace' => 'caseno'));
$valid = $securimage->check($_POST['caseno_captcha']);

		/*Validate Session and User Captcha Values*/
		if ($valid){ 		
			
if($_POST['submit'] && $_POST['cause_list_dt']!='' && $_POST['case_type']!='' && $_POST['case_no']!='' && $_POST['case_year']!='')
{
...
else if(empty($case_type))
   {
        echo '<br><h3 style="color:red;text-align:center;">Enter your Case Type!</h3>';
     
   }
 else if(!empty($case_type) && $validator->chkbadchar($case_type) == false)
 {
	  echo '<br><h3 style="color:red;text-align:center;">Please enter valid Case Type</h3>';

 } 
  else if(!empty($case_type) && $validator->test_datatype($case_type,"[^0-9]") == false)
 {
  echo '<br><h3 style="color:red;text-align:center;">Please enter valid  Numeric  Case Type</h3>';
 }
 else if(empty($case_num))
   {
        echo '<br><h3 style="color:red;text-align:center;">Enter your Case Number!</h3>';
     
   }
 else if(!empty($case_num) && $validator->chkbadchar($case_num) == false)
 {
	  echo '<br><h3 style="color:red;text-align:center;">Please enter valid Case Number</h3>';

 } 
  else if(!empty($case_num) && $validator->test_datatype($case_num,"[^0-9]") == false)
 {
  echo '<br><h3 style="color:red;text-align:center;">Please enter valid  Numeric  Case Number</h3>';
 }
 else if(empty($case_year))
   {
        echo '<br><h3 style="color:red;text-align:center;">Enter your Case Year!</h3>';
     
   }
 else if(!empty($case_year) && $validator->chkbadchar($case_year) == false)
 {
	  echo '<br><h3 style="color:red;text-align:center;">Please enter valid Case Year</h3>';

 } 
  else if(!empty($case_year) && $validator->test_datatype($case_year,"[^0-9]") == false)
 {
  echo '<br><h3 style="color:red;text-align:center;">Please enter valid  Numeric  Case Year</h3>';
 }
 else
 {	
...
//echo "SELECT for_bench_id FROM cause_list WHERE cause_case_type ='".$case_type."' and cause_reg_no ='".$case_num."' and cause_reg_year ='".$case_year."' and causelist_date='".$causelist_date."' ";

$jud_bench_qry=$MDU_HCMAS_DB->query("SELECT for_bench_id FROM cause_list WHERE cause_case_type ='".$case_type."' and cause_reg_no ='".$case_num."' and cause_reg_year ='".$case_year."' and causelist_date='".$causelist_date."' AND causelist_sr_no!='0' AND  originalsr_no!='0' and elimination='N' UNION SELECT for_bench_id FROM cause_list_a WHERE cause_case_type ='".$case_type."' and cause_reg_no ='".$case_num."' and cause_reg_year ='".$case_year."' and causelist_date='".$causelist_date."' AND causelist_sr_no!='0' AND  originalsr_no!='0' and elimination='N'");
$jud_bench_row=$jud_bench_qry->fetch();
$jud_bench =$jud_bench_row['for_bench_id'];
if($jud_bench)
{
...
?>
				...
				<h2 class="post-title" align="center">MADURAI BENCH OF MADRAS HIGH COURT</h2>
				...
				<h6 class="post-title" style='color:red' align="center">SEARCH RESULTS FOR  : <?php echo $case_details_valu;?></h6>
...
<table id="case" class="display responsive nowrap" style="width:100%">
      ...
      <th>Current Position</th>
		...
		<?php
		 ...
		 $cause_list=$MDU_HCMAS_DB->query("SELECT distinct(sr_no),cino,case_no,ctype,originalsr_no,case_remark,causelist_type,causelist_period,list_from_date,list_to_date,causelist_date,for_bench_id,purpose_cd,reg_dt,filing_dt,section_id,on_filing,clink_code,elimination,causelist_sr_no,cause_list_eliminate,ia_no,ia_next_date,ia_flag,search_case,initial_status,final_status,cause_case_type,cause_reg_no,cause_reg_year,ia_case_type,bunch_code,purpose_priority,short_order,court_no FROM cause_list WHERE  causelist_date ='".$causelist_date."' AND for_bench_id IN (".$jud_bench.") AND cause_case_type ='".$case_type."' and cause_reg_no ='".$case_num."' and cause_reg_year ='".$case_year."' and causelist_sr_no!='0' AND  originalsr_no!='0' and elimination='N'  UNION SELECT distinct(sr_no),cino,case_no,ctype,originalsr_no,case_remark,causelist_type,causelist_period,list_from_date,list_to_date,causelist_date,for_bench_id,purpose_cd,reg_dt,filing_dt,section_id,on_filing,clink_code,elimination,causelist_sr_no,cause_list_eliminate,ia_no,ia_next_date,ia_flag,search_case,initial_status,final_status,cause_case_type,cause_reg_no,cause_reg_year,ia_case_type,bunch_code,purpose_priority,short_order,court_no FROM cause_list_a WHERE  causelist_date ='".$causelist_date."' AND for_bench_id IN (".$jud_bench.") AND cause_case_type ='".$case_type."' and cause_reg_no ='".$case_num."' and cause_reg_year ='".$case_year."' and causelist_sr_no!='0' AND  originalsr_no!='0' and elimination='N' ORDER BY sr_no");
...
$purpose_cd=$cause_list_row['purpose_cd'];
 ...
 $query=$MDU_HCMAS_DB->query("select a.judge_code,a.judge_name as judge_name,a.desg_code,a.short_judge_name from Judge_name_t as a,judge_t as b 
	...
	while($row1=$query->fetch())
	{
		...
		$judgrows++;
					...
					//IA CASE Details 
					$ia_case_qry=$MDU_HCMAS_DB->query("SELECT appotherparty,appotheradv,againotheradv,case_no,purpose_code,case_remark FROM ia_filing WHERE case_no = '".$ia_case_no."' and ia_no='".$ia_num."' union SELECT appotherparty,appotheradv,againotheradv,case_no,purpose_code,case_remark FROM ia_filing_a WHERE case_no = '".$ia_case_no."' and ia_no='".$ia_num."'");
			...
			else
			{
                ...
                <td>'.$purpose_name.'</td>
					...
					$link_cause='';
					 ...
					 $link_cause_list_qry=$MDU_HCMAS_DB->query("SELECT cino,cause_case_type,cause_reg_no,cause_reg_year,on_filing,case_no,causelist_type FROM cause_list WHERE causelist_date ='".$causelist_date."'  AND for_bench_id = '".$for_bench_id."'  AND causelist_type='".$causelist_id."' AND clink_code ='".$case_no."' AND  elimination='N' ORDER BY sr_no");
			...
			while($link_cause_list_row=$link_cause_list_qry->fetch())	
			{
				...
				$link_civil_qry = $MDU_HCMAS_DB->prepare("SELECT filcase_type,fil_no,fil_year,pet_name,res_name,pet_adv_cd,res_adv_cd,regcase_type,reg_no,reg_year,
				...
				if($link_cause_list_row['on_filing']=='Y')
				{
					...
					$link_case_details="SR No. ".$link_fil_type.'.'.$link_RegCase_no.'/'.$link_RegCase_year;
					 ...
					 $link_civil_t_a_qry = $MDU_HCMAS_DB->query("SELECT date_filing_disp,disp_nature FROM civil_t_a where cino='".$link_cino."' and date_filing_disp IS NOT NULL  ORDER BY date_filing_disp DESC");
						 ...
						 $link_todays_date=date('d-M-Y',strtotime($disp_c_nat['date_filing_disp']));
						  ...
						  {
							  $link_resSrCaseType = $MDU_HCMAS_DB->query("SELECT todays_date,order_remark,next_date FROM daily_proc_filing where cino='".$link_cino."' and todays_date IS NOT NULL  ORDER BY todays_date DESC");
				...
				else
				{
					...
					$link_daily_pro = $MDU_HCMAS_DB->query("SELECT adjcode, srno,todays_date,order_remark,next_date FROM daily_proc where cino='".$link_cino."' and todays_date IS NOT NULL  UNION SELECT adjcode, srno,todays_date,order_remark,next_date FROM daily_proc_a where cino='".$link_cino."' and todays_date IS NOT NULL  ORDER BY todays_date DESC");
					...
					{
						$link_disposal_qry = $MDU_HCMAS_DB->query("SELECT order_remark FROM disposal_proc where cino='".$link_cino."' and todays_date IS NOT NULL  ORDER BY todays_date DESC");
							if($link_disposal_qry->rowcount()>0)
						  {
							  $link_disposal_row = $link_disposal_qry->fetch();
							   
						$link_dis_pos_case=$MDU_HCMAS_DB->prepare("SELECT disp_nature,date_of_decision FROM civil_t_a  where cino=:cino and date_of_decision IS NOT NULL  ORDER BY date_of_decision DESC");
						...
						$link_todays_date='';	
						  ...
						  {
							   $link_civil_t_query1 = $MDU_HCMAS_DB->query("SELECT disp_nature,date_of_decision FROM civil_t_a where cino='".$link_cino."' and date_of_decision IS NOT NULL  ORDER BY date_of_decision DESC");
						  ...
						  {
							  $link_restore_qry=$MDU_HCMAS_DB->prepare("SELECT remark,next_date,todays_date FROM restorerevoke  where cino=:cino  AND datetype='rev' and todays_date IS NOT NULL  ORDER BY todays_date DESC");
			...
			else
			{
				...
				$civil_qry = $MDU_HCMAS_DB->prepare("SELECT filcase_type,fil_no,fil_year,pet_name,res_name,pet_adv_cd,res_adv_cd,regcase_type,reg_no,reg_year,
				...
				if($cause_list_row['on_filing']=='Y')
				{
					...
					$civil_t_a_qry = $MDU_HCMAS_DB->query("SELECT date_filing_disp,disp_nature FROM civil_t_a where cino='".$cino."' and date_filing_disp IS NOT NULL  ORDER BY date_filing_disp DESC");
						 ...
						 $todays_date='';
						  ...
						  {
							  $resSrCaseType = $MDU_HCMAS_DB->query("SELECT todays_date,order_remark,next_date FROM daily_proc_filing where cino='".$link_cino."' and todays_date IS NOT NULL  ORDER BY todays_date DESC");
				...
				else
				{
					...
					//echo "SELECT adjcode, srno FROM daily_proc where cino='".$cino."' and todays_date='".$causelist_date."' UNION SELECT adjcode, srno FROM daily_proc_a where cino='".$cino."' and todays_date='".$causelist_date."' ORDER BY srno DESC";
					 $daily_pro = $MDU_HCMAS_DB->query("SELECT adjcode, srno,todays_date,order_remark,next_date FROM daily_proc where cino='".$cino."' and todays_date IS NOT NULL  UNION SELECT adjcode, srno,todays_date,order_remark,next_date FROM daily_proc_a where cino='".$cino."' and todays_date IS NOT NULL  ORDER BY todays_date DESC");
					...
					{
						$disposal_qry = $MDU_HCMAS_DB->query("SELECT order_remark FROM disposal_proc where cino='".$cino."' and todays_date IS NOT NULL  ORDER BY todays_date DESC");
							if($disposal_qry->rowcount()>0)
						  {
							  $disposal_row = $disposal_qry->fetch();
							   
						$dis_pos_case=$MDU_HCMAS_DB->prepare("SELECT disp_nature,date_of_decision FROM civil_t_a  where cino=:cino and date_of_decision IS NOT NULL  ORDER BY date_of_decision DESC");
						...
						$todays_date='';	
						  ...
						  {
							   $civil_t_query1 = $MDU_HCMAS_DB->query("SELECT disp_nature,date_of_decision FROM civil_t_a where cino='".$cino."' and date_of_decision IS NOT NULL  ORDER BY date_of_decision DESC");
						  ...
						  {
							  $restore_qry=$MDU_HCMAS_DB->prepare("SELECT remark,next_date,todays_date FROM restorerevoke  where cino=:cino  AND datetype='rev' and todays_date IS NOT NULL  ORDER BY todays_date DESC");
