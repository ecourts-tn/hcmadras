...
<div class="pad group">
<h2 class="post-title" align="center">Former Puisne Judges</h2><br>
 
 <table id="table_id2"  >
    <thead>
        <tr>
            <th> Former Puisne Judges</th>            
        </tr>
    </thead>
    <tbody>
	...
	//$sql1 = $DB_con->query("select * from judges where j_display='Y'  and  j_page='FC' order by j_sen asc");
	$sql1 = $DB_con->query("select * from former_judges where j_display='Y'  order by j_sen desc");
	...
	$name=$row_jud['j_name'];
	$fj_fyr=$row_jud['fj_fyr'];
	$fj_tyr=$row_jud['fj_tyr'];
	$period='';
					if( !is_null($fj_fyr) and $fj_fyr!='' )
						$period=$fj_fyr.' - ';
					if( !is_null($fj_tyr) and $fj_tyr!='' )
						$period.=$fj_tyr;
   ...
   data-content="<div><img src='admin/view_image.php?img_id=<?php echo base64_encode($row_jud['j_id'])?>&page=<?php echo base64_encode('FJ'); ?>'  width='250' height='250' align='center'></div> <p><?php echo nl2br(html_entity_decode($row_jud['j_profile']));?></p><br><button type='button' class='btn btn-primary' data-toggle='popover' style='float:center'>Close</button>"><img style="cursor: pointer;" width="75" height="75" src="admin/view_image.php?img_id=<?php echo base64_encode($row_jud['j_id'])?>&page_id=<?php echo base64_encode('FJ'); ?>&page=<?php echo base64_encode('FJ'); ?>" title="Click to Enlarge" alt="<?php echo $hon.$prefi.".Justice ".$name.$fjudge; ?>" id="judge_thumb" /></a>
            </div> 
        
			<div class="col-md-8">
			  <div class="judge_container">
				<div class="jname_text"><span style="font-weight:bold !important">	
				<?php echo $hon.$prefi.".Justice ".$name.$fjudge; ?> </span>
				<br/><em style="font-size:14px"> <?php echo $period; ?> </em>
