...
require('config/dbconfig_status.php');
	...
	curl_close($ch);
	$fil_nme = $ord_yr.'_'.$ord_caseno.'_'.$ord_no.'_MDUcnr';
		...
		if( isset($_POST['cnrno_captcha']) && !empty($_POST['cnrno_captcha']) ){
			...
			if($valid){
				...
				if($cnr_no ){
					...
					$query_main = $MDU_HCMAS_DB->prepare("SELECT * FROM civil_t WHERE cino = :cino  union SELECT * FROM civil_t_a WHERE cino = :cino   ORDER BY reg_no ASC ");
				...
				if($result){
					...
					if($query_main->rowcount()){
						...
						$main_srcase_typeno =  trim($query_main_result['filcase_type']) ? trim($query_main_result['filcase_type']) : '' ;
							...
							//getting case no. and reg no and reg year 
...
<?php
	...
	echo '<td class="hidden-lg hidden-md">Case No.</td><td class="result_td" > '.$wmp_cses_typ_name.'.'.$wmp_cases_result['reg_no'].'/'.$wmp_cases_result['reg_year'].' </td>';
