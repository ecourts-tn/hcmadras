...
require('config/dbconfig_status.php');
	...
	curl_close($ch);
	$fil_nme = $ord_yr.'_'.$ord_caseno.'_'.$ord_no.'_MHCcnr';
...
<?php	
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		if( isset($_POST['cnrno_captcha']) && !empty($_POST['cnrno_captcha']) ){
			
			/* $user_captcha = check_input($_POST['filingno_captcha']);
			$page_captcha = trim($_SESSION['filingno_captcha']); */
			
			$securimage = new Securimage(array('namespace' => 'cnrform'));
            $valid = $securimage->check($_POST['cnrno_captcha']);
			
			
			if($valid){

					if(isset($_POST['cnrno_case_nme']) && !empty($_POST['cnrno_case_nme']) ) {
			
			 
						$cnr_no =   check_input($_POST['cnrno_case_nme']);
			
				if($cnr_no ){
				
					$query_main = $HCMAS_DB->prepare("SELECT * FROM civil_t WHERE cino = :cino  union SELECT * FROM civil_t_a WHERE cino = :cino   ORDER BY reg_no ASC ");
			 	
			
				$result = $query_main->execute(array(':cino'=>$cnr_no));	
					...
					if($query_main->rowcount()){
						...
						$main_srcase_typeno =  trim($query_main_result['filcase_type']) ? trim($query_main_result['filcase_type']) : '' ;
							...
							//getting fullform main case  type
								...
								$main_case_no = $query_main_result['reg_no'] ? trim($query_main_result['reg_no']) : '';
								$main_case_year =  $query_main_result['reg_year'] ? trim($query_main_result['reg_year']) : '';
...
<?php					
						...
						/*echo check_tags(strtoupper(trim($query_main_result['pet_name']))).$party_inpers.'<br>'; 
...
<?php
	...
	$wmp_cass ='';
	if($query_main_result['main_case_no']){
...
<?php
if($query_main_result['link_cino']){
...
<?php
				$prayer_res ='';
					if($query_main_result['branch_id'])		{
...
<?php
    ...
    if(trim($query_main_result['cino'])){
		...
		$srrecds = ''; 
		$regnoo = $query_main_result['reg_no'] ? trim($query_main_result['reg_no']) : '';
		if(!$regnoo){
		...
		if($query_histry_herng){
			...
			if($query_histry_herng->rowcount()){
				...
				//echo "<tr> <td align='center' colspan ='7'> No records </td><tr>";
			} //row count
		} //query not working
	} //cino emtpy
...
<?php
		...
		if($main_case_typeno  and $main_case_no and $main_case_year){
			
			$noticess_qry = $CIS_DB->prepare("SELECT * FROM tn_notice_servestatus where regcase_type = :casenam_e and reg_no = :case_no_o and reg_year = :case_ye_ar  and casenotype = 'R' and despatch_entry_date IS NOT NULL  and display='Y'  order by party_no,type,process_id");
		...
		if($noticess_qry){
			$noticess_qry->execute(array(':casenam_e'=>$main_case_typeno,':case_no_o'=>$main_case_no,':case_ye_ar'=>$main_case_year));
