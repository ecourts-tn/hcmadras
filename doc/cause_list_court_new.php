...
$court_num='';
 $cause_list_main_val='';
 //echo  "SELECT causelist_sr_no,for_bench_id,causelist_id,causelist_date,court_id FROM causelist_title WHERE cause_list_status='P' AND court_id='".$court_no."' AND causelist_date ='".$causelist_date."' order by causelist_id ";
 $cause_list_title=$HCMAS_DB->query("SELECT causelist_sr_no,for_bench_id,causelist_id,causelist_date,court_id FROM causelist_title WHERE cause_list_status='P' AND court_id='".$court_no."' AND causelist_date ='".$causelist_date."' order by causelist_id ");
 while($cause_title_row=$cause_list_title->fetch())
 {
 $causelist_sr_no=$cause_title_row['causelist_sr_no'];
 $causelist_date=$cause_title_row['causelist_date'];
 $for_bench_id=$cause_title_row['for_bench_id'];
 $causelist_id=$cause_title_row['causelist_id'];
 //$find_court_no=sprintf("%02d",rtrim($cause_title_row['court_id']));
 $find_court_no=$cause_title_row['court_id'];
 $court_id=str_replace("VC","",strtoupper($cause_title_row['court_id']));
	...
	while($row1=$query->fetch())
	{
		...
		$judgrows++;
					...
					$section = $getdata->getsection($causelist_id);
						...
						$court_num='<tr><td colspan="6" style="color:#f4f7f4;font-weight: bold;background-color: #5a5a57;">COURT No. : '.$find_court_no.'</td><tr>';
					$cause_list_type='<tr><td colspan="6" style="color:#f4f7f4;font-weight: bold;background-color: #5a5a57;">'.$section.'</td><tr>';
					 $jud_data='<tr><td colspan="6" style="color:#f4f7f4;font-weight: bold;background-color: #5a5a57;">'.$judges.'</td><tr>';
					
					//error_log("SELECT distinct(purpose_cd),purpose_priority FROM cause_list WHERE causelist_type='".$causelist_id."' AND causelist_date ='".$causelist_date."' AND for_bench_id = '".$for_bench_id."' AND causelist_sr_no='".$causelist_sr_no."' and originalsr_no!='0' and elimination='N'  and purpose_cd!='0' ORDER BY purpose_priority");
					
					$cause_list_main_val='';
					$purpose_cause_list=$HCMAS_DB->query("SELECT distinct(purpose_cd),purpose_priority FROM cause_list WHERE causelist_type='".$causelist_id."' AND causelist_date ='".$causelist_date."' AND for_bench_id = '".$for_bench_id."' AND causelist_sr_no='".$causelist_sr_no."' and originalsr_no!='0' and elimination='N'  and purpose_cd!='0'  ORDER BY purpose_priority");
				
				while($purpose_row=$purpose_cause_list->fetch())
				{
					$purpose_cd=$purpose_row['purpose_cd'];
					
					$purpose_name = $getdata->getpurpose($purpose_cd);
					//Cause list DataTable
					$cause_list_data='';
			
			$srno=1;		
   $cause_list=$HCMAS_DB->query("SELECT distinct(sr_no),cino,case_no,ctype,originalsr_no,case_remark,causelist_type,causelist_period,list_from_date,list_to_date,causelist_date,for_bench_id,purpose_cd,reg_dt,filing_dt,section_id,on_filing,clink_code,elimination,causelist_sr_no,cause_list_eliminate,ia_no,ia_next_date,ia_flag,search_case,initial_status,final_status,cause_case_type,cause_reg_no,cause_reg_year,ia_case_type,bunch_code,purpose_priority,short_order,court_no FROM cause_list WHERE causelist_type='".$causelist_id."' AND causelist_date ='".$causelist_date."' AND for_bench_id = '".$for_bench_id."' AND causelist_sr_no='".$causelist_sr_no."' and purpose_cd='".$purpose_cd."' AND  originalsr_no!='0' and elimination='N'  ORDER BY sr_no");
			...
			else
			{
				...
				$ia_extra_case='<tr>
					...
					$link_cause='';
					 ...
					 $link_cause_list_qry=$HCMAS_DB->query("SELECT cino,cause_case_type,cause_reg_no,cause_reg_year,on_filing,case_no FROM cause_list WHERE causelist_date ='".$causelist_date."'  AND for_bench_id = '".$for_bench_id."'  AND causelist_type='".$causelist_id."' AND clink_code ='".$case_no."' AND  elimination='N' ORDER BY sr_no");
			...
			else
			{
				$res_cont="";
					...
					$cause_list_data .=$main_cause.$link_cause;
					$srno++;
				}
				}
			}
			$cause_list_main_val .='<tr><td colspan="6" style="color:#f4f7f4;font-weight: bold;background-color:#a00d0d;">'.$purpose_name.'</td></tr>'.$cause_list_data.$ia_case;
			 
 }
 
  $data_val .= $jud_data.$cause_list_type.'<tr><td colspan="6"></td></tr>'.$cause_list_main_val;
 }

  echo $data= $court_num.$data_val;
 }
?>
