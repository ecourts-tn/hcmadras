...
function ChkFileExists($ord_yr,$ord_caseno,$ord_no,$bench_type) {
	$order_yr=$ord_yr;
...
else if(isset($_POST['party_name']) and $_POST['party_name']!='' and isset($_POST['from_date']) and $_POST['from_date']!='')
{
	$securimage2 = new Securimage(array('namespace' => 'partyform'));
    $valid2 = $securimage2->check($_POST['party_captcha']);
	$temp_id="party";
}
else if(isset($_POST['judge_name']) and $_POST['judge_name']!='')
{
	$securimage3 = new Securimage(array('namespace' => 'judgeform'));
    $valid3 = $securimage3->check($_POST['judge_captcha']);
	$temp_id="judge";
}	
else if(isset($_POST['order_date']) and $_POST['order_date']!='' )
{
...
<table id="<?php echo 'judment_'.$temp_id; ?>" class="display responsive nowrap" cellspacing="0" width="100%" >
  ...
  <thead style="background: #827676; color: #fff;">
    <tr>
      ...
      <th scope="col" width="5%" style="text-align:left;"> Bench </th>      
...
if(isset($_POST['RegCase_type']) and $_POST['RegCase_type']!='' and is_numeric($_POST['RegCase_type']))
{
	...
	if($valid1){  
		...
		$query_main = $DB_option[$db]->query("SELECT * FROM civil_t WHERE regcase_type = '".$case_type."' AND reg_no = '".$case_num."' AND reg_year = '".$case_year."' union SELECT * FROM civil_t_a WHERE regcase_type = '".$case_type."' AND reg_no = '".$case_num."' AND reg_year = '".$case_year."' ORDER BY reg_no ASC ");
...
if(isset($_POST['judge_name']) and $_POST['judge_name']!='')
{
	...
	*/
	if($valid3){  
		...
		$query_main = $DB_option[$db]->query("SELECT * FROM judge_name_t WHERE judge_name ilike '%".$judge_name."%' and display='Y'");
...
<?php
	...
	if($bench=='ALL' && $db=='0'){
		$bench1='Principal Seat';
		$bench_type='MHC';
	}
	else if($bench=='ALL' && $db=='1'){
		$bench1='Madurai Bench';
		$bench_type='MDU';
	}
	else{
		if($bench=='MHC'){
		...
		$order_qry=$cond;
		$order_qry_exe =$DB_option[$db]->query($order_qry);
		...
		while($order_row = $order_qry_exe->fetch())
		{
			...
			$case_det_qry = $DB_option[$db]->query("SELECT * FROM civil_t WHERE cino = '".$order_cino."' union SELECT * FROM civil_t_a WHERE cino = '".$order_cino."'");
			...
			$reg_case_type = $getdata->getcasetype($case_det_row['filcase_type']);
			 $case_details_valu = $reg_case_type.'.'.$case_det_row['fil_no'].'/'.$case_det_row['fil_year'].'<br>(filing stage)';
				...
				//echo "<br/>its if: select a.judge_code,a.judge_name as judge_name,a.desg_code,a.short_judge_name from Judge_name_t as a,judge_t as b where a.display='Y' and a.judge_code=b.judge_code and b.court_no='".$order_court_no."' and from_dt IS NOT NULL order by b.judge_priority";
					$query = $DB_option[$db]->query("select a.judge_code,a.judge_name as judge_name,a.desg_code,a.short_judge_name from Judge_name_t as a,judge_t as b where a.display='Y' and a.judge_code=b.judge_code and b.court_no='".$order_court_no."' and from_dt IS NOT NULL order by b.judge_priority");
				...
				else 
				{
					...
					if(($orjc1))
					{
						...
						$query=$DB_option[$db]->query("select * from judge_name_t where judge_code in(".rtrim($order_jocode,",").") and display='Y' order by judge_priority  ");
						...
						$query = $DB_option[$db]->query("select * from judge_name_t where judge_code='".$order_jocode."' and display='Y' "); 
					}
				} //else 	
				...
				//echo $case_details_valu."-".$order_jocode."-".$file_name."-".ChkFileExists($file_name);
					...
					$file_name=ChkFileExists($year_cino,$order_caseno,$order_no,$bench_type);
					if($file_name!='404')
	 	            {
				...
				<tr class="<?php echo $file_name;?>" id="<?php echo $res_cino;?>">
				   ...
				   <td style="text-align:left;"><?php echo $case_details_valu;?></td> 
				    <td style="text-align:left;"><?php echo $bench1;?></td>
...
{
	echo '<tr colspan="8"><h3 style="color:red;">Captcha not matching</h3></tr>';
}
else
	echo '<tr colspan="8"><h3 style="color:red;">NO SEARCH RESULTS FOUND</h3></tr>';
}
?>
 </tbody>
</table>
