...
$getdata =new GETDETAILS($HCMAS_DB);
	...
	curl_close($ch);
	$fil_nme = $ord_yr.'_'.$ord_caseno.'_'.$ord_no.'_MHCmain';
			...
			if($valid){
				...
				if(isset($_POST['case_nme']) && !empty($_POST['case_nme']) &&  isset($_POST['case_no']) && !empty($_POST['case_no']) && isset($_POST['case_year']) && !empty($_POST['case_year'])){
					...
					if($case_name  && $case_no && $case_year){
						...
						$query_main = $HCMAS_DB->prepare("SELECT * FROM civil_t WHERE regcase_type = :case_name AND reg_no = :caseno AND reg_year = :case_year union SELECT * FROM civil_t_a WHERE regcase_type = :case_name AND reg_no = :caseno AND reg_year = :case_year  ORDER BY reg_no ASC ");
				...
				if($conncted_cases->rowCount()){
					...
					while($conncted_cases_result =$conncted_cases->fetch()){
						...
						if(trim($conncted_cases_result['regcase_type']) and trim($conncted_cases_result['reg_no']) and trim($conncted_cases_result['reg_year']))
						{
							...
							$connctd_wmp = $HCMAS_DB->query("select * from civil_t where main_case_no ='".$conncted_cases_result['case_no']."' union select * from civil_t_a where main_case_no ='".$conncted_cases_result['case_no']."' ");
								...
								if($connctd_wmp->rowCount()){
									while($connctd_wmp_result = $connctd_wmp->fetch()){
										...
										if($connctd_wmp_result['regcase_type']){
											$conectd_wmp_type = $HCMAS_DB->query("select type_name from case_type_t WHERE case_type ='".$connctd_wmp_result['regcase_type']."'");
						...
						if(!trim($order_jocode)){
							...
							if($orjc1)
							{
								...
								if($order_jocodes){
									...
									$query = $HCMAS_DB->query("select a.judge_code,a.judge_name as judge_name,a.desg_code,a.short_judge_name from Judge_name_t as a,judge_t as b where a.display='Y' and a.judge_code=b.judge_code and b.court_no='".$order_court_no."' and from_dt IS NOT NULL order by b.judge_priority asc");
								}
							} else {
...
?>	
				...
				<tr class="<?php echo $file_name;?>" id="<?php echo $order_cino;?>">
				 ...
				 <th class='hidden-lg hidden-md'>Petitioner Name</th>  <td style="text-align:left;"><?php echo check_tags(strtoupper(trim($query_main_result['pet_name'])));?></td>
				  ...
				  <th class='hidden-lg hidden-md'>Judge name</th> <td style="font-weight:bold;text-align:left;"><?php echo $judges.$chk;?></td>
				  <th class='hidden-lg hidden-md'>Order Copy</th> 
				   <td style="text-align:center;">
				    <form method="POST" action="order_view_ftp.php" target="_blank" >
				  ...
				  <!--<td style="text-align:center;"><a href="order_view_ftp.php?fileName=<?php //echo base64_encode($file_name);?>" target="_blank"  class="text-danger pdf_label"><i class="fa fa-file-pdf-o"></i>&nbsp;PDF </a></td>  -->          				   
		           <!--<td style="text-align:center;"><a href="pdffile/sample.pdf" target="_blank" onclick="#" class="text-danger pdf_label"><i class="fa fa-file-pdf-o"></i>&nbsp;PDF </a></td>	-->	           
				</tr>						  
<?php
		       $sno++;
						}
