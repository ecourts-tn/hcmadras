...
require('config/dbconfig_status.php');
	...
	curl_close($ch);
	$fil_nme = $ord_yr.'_'.$ord_caseno.'_'.$ord_no.'_MHCfil';
...
<?php	
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		if( isset($_POST['filing_captcha']) && !empty($_POST['filing_captcha']) ){
			
			/* $user_captcha = check_input($_POST['filingno_captcha']);
			$page_captcha = trim($_SESSION['filingno_captcha']); */
			
			$securimage = new Securimage(array('namespace' => 'comments'));
            $valid = $securimage->check($_POST['filing_captcha']);
			
			
			if($valid){

				if(isset($_POST['fil_case_nme']) && !empty($_POST['fil_case_nme']) &&  isset($_POST['fil_case_no']) && !empty($_POST['fil_case_no']) && isset($_POST['fil_case_year']) && !empty($_POST['fil_case_year']) ) {
			
			$cas_name =  check_input($_POST['fil_case_nme']);
			$cas_no =  check_input($_POST['fil_case_no']);
			$cas_year =   check_input($_POST['fil_case_year']);
			...
			if($case_name  && $case_no && $case_year){
				...
				$query_main = $HCMAS_DB->prepare("SELECT * FROM civil_t WHERE filcase_type = :case_name AND fil_no = :caseno AND fil_year = :case_year union SELECT * FROM civil_t_a WHERE filcase_type = :case_name AND fil_no = :caseno AND fil_year = :case_year  ORDER BY reg_no ASC ");
					...
					if($query_main->rowcount()){
						...
						$main_srcase_typeno =  trim($query_main_result['filcase_type']) ? trim($query_main_result['filcase_type']) : '' ;
							...
							//getting fullform sr case type
								...
								$sr_casetype = $HCMAS_DB->prepare("select type_name,full_form from case_type_t where case_type = :srcasetype_no ");
							...
							$main_srcase_no = trim($query_main_result['fil_no']) ? trim($query_main_result['fil_no']) : '' ;
							$main_srcase_year = trim($query_main_result['fil_year']) ? trim($query_main_result['fil_year']) : '';
							...
							//getting fullform main case type
								...
								$regno_casetype = $HCMAS_DB->prepare("select type_name,full_form from case_type_t where case_type = :regcasetype_no ");
								if($regno_casetype){
							...
							//getting fullform main case  type
								...
								$main_case_no = trim($query_main_result['reg_no']) ? trim($query_main_result['reg_no']) : '';
								$main_case_year =  trim($query_main_result['reg_year']) ? trim($query_main_result['reg_year']) : '';
							...
							if($cino){
								...
								$main_peti_address_qry = $HCMAS_DB->prepare("select address,dist_code from party_address where cino = :cino AND party_no = '0' AND type = '1' union select address,dist_code from party_address_a where cino = :cino  AND party_no = '0' AND type = '1' ");
									...
									if($main_peti_addres_execute){
										if($main_peti_address_qry->rowcount()){
											...
											if($peti_disct_cd ){ 
												...
												$peti_distr_qry = $HCMAS_DB->prepare("select dist_name from district_t where dist_code = :distrct_cd ");
												if($peti_distr_qry ){
						...
						if($cino){
							...
							$main_resp_address_qry = $HCMAS_DB->prepare("select address,dist_code from party_address where cino =:cino AND party_no = '0' AND type = '2' union select address,dist_code from party_address_a where cino= :cino AND party_no = '0' AND type = '2'  ");
								...
								if($main_resp_address_execu){
									if($main_resp_address_qry->rowcount()){
										...
										if($resp_disct_cd){
											...
											$resp_disct_qry = $HCMAS_DB->prepare('select dist_name  from district_t where dist_code = :distcode');
											if($resp_disct_qry){
...
?>						
		...
		<table   class="table_caseno_search" >	
			<tbody>
				<tr>
					<th> Filing No.  </th>
					<td>   
...
<?php
						...
						if(trim($query_main_result['filcase_type']) =='19'){
							...
							$addinl_qry = $HCMAS_DB->prepare("select * from additional_info where cino = :cino and search21 ='1'");
...
<?php 
							if($query_main_result['date_of_decision']=='' && $query_main_result['date_filing_disp']=='' ){
								...
								if(trim($query_main_result['disp_nature'])){
									$dispsl_natre = $HCMAS_DB->prepare("select disp_name from disp_type_t where disp_type = :disple_nature and disp_type!=0");
								...
								echo " by ";
									...
									$disposl_proc =   $HCMAS_DB->prepare("SELECT judge_code FROM disposal_proc WHERE cino = :cino");
									...
									if($dispsl_judg_nm){
										...
										foreach($judge_displ as $code_vallue){
											if($code_vallue){
												$judg_names_disposl = $HCMAS_DB->prepare("select judge_name from judge_name_t where judge_code =:jj_code");
										...
										foreach ($judg_namees as $value){
											if($value){
												$judg_names = $HCMAS_DB->prepare("select judge_name from judge_name_t where judge_code =:judd_code");
										...
										$court_codde = $HCMAS_DB->prepare("select * from  judge_t where court_no = :jud_courtcode ");
										...
										if($court_codde){
											...
											if($court_codde->rowcount()){
												while($court_codde_res = $court_codde->fetch()){
													...
													if($judge_nmme_cod){
														$judge_nme_court = $HCMAS_DB->prepare("select * from judge_name_t where judge_code =  :judnmee_code");
														if($judge_nme_court){
									...
									if(!trim($judg_nmes)){
										...
										if($cino){
											...
											$sr_judge = $HCMAS_DB->prepare("select court_no from disposal_proc_filing  where cino =:cino");
											//var_dump($sr_judge);
											if($sr_judge){
												...
												if($sr_judge->rowCount()){
													...
													if($court_code){
														$juedg_cdee = $HCMAS_DB->prepare("select judge_code from judge_t	 where court_no = :coourt_code ");
														//var_dump($juedg_cdee);
														if($juedg_cdee){
															...
															if($juedg_cdee->rowCount()){
																...
																if($judg_cood){
																	$juddg_name = $HCMAS_DB->prepare("select judge_name from judge_name_t where judge_code = :juddgg_code");
...
?>						
				...
				<tr  class="hidden-sm"  > <th class="text-center" colspan="4"> Petitioner and Respondent Details</th> </tr>
...
<?php					
						...
						if(trim($query_main_result['cino'])){
							if($reg_casetype_short_form==119){
								$addinl_inf = $HCMAS_DB->prepare("select * from additional_info where cino = :cino");
								if($addinl_inf){
						...
						if(isset($query_main_result['hide_pet_name'])||isset($query_main_result['hide_partyname'])){
							...
							if(trim($query_main_result['hide_pet_name'])=='Y' || trim($query_main_result['hide_partyname'])=='Y'){
								...
								$pet_det = check_tags(strtoupper(trim($query_main_result['pet_name']))).'<br>'.$peti_address.' '.$peti_disct_nm.'<br>';
						}
						else
							$pet_det = check_tags(strtoupper(trim($query_main_result['pet_name']))).'<br>'.$peti_address.' '.$peti_disct_nm.'<br>';
						
						echo $pet_det; 
						/*echo check_tags(strtoupper(trim($query_main_result['pet_name']))).'<br>'; 
						...
						if($cino && isset($query_main_result['hide_partyname']) && trim($query_main_result['hide_partyname'])!='Y'){
							...
							$addl_petitioner = $HCMAS_DB->prepare("(select CAST(party_id as INTEGER ) as one,* from civ_address_t where cino = :cino and display='Y' and type='1' ) union ( select CAST(party_id as INTEGER ) as one,* from civ_address_t_a where cino = :cino and display='Y' and type='1' ) order by one");
							...
							if($addl_petitioner){
								...
								if($addl_petitioner->rowcount()){
									...
									while($addl_petitioner_result = $addl_petitioner->fetch()){
										...
										$addl_petitioner_addrs =  $HCMAS_DB->prepare("select address,dist_code from party_address where cino= :cino AND party_no = :partey_no AND type = '1' union select address,dist_code from party_address_a where cino= :cino AND party_no = :partey_no AND type = '1'");
										...
										if($addl_petitioner_addrs){
											...
											if($addl_petitioner_addrs->rowcount()){
												...
												if($adl_peti_dst_cd){
													...
													$addl_petitioner_distr = $HCMAS_DB->prepare("select dist_name from district_t where dist_code = :addl_peti_discd");
...
<?php						
						...
						if(isset($query_main_result['hide_res_name'])||isset($query_main_result['hide_partyname'])){
							...
							if(trim($query_main_result['hide_res_name'])=='Y' || trim($query_main_result['hide_partyname'])=='Y'){
								...
								$res_det = check_tags(strtoupper(trim($query_main_result['res_name']))).$party_inpers_resp.'<br>'.check_tags(trim($resp_address)).' '.$resp_disct_nm.'<br><br>' ; 
						}
						else
							$res_det = check_tags(strtoupper(trim($query_main_result['res_name']))).$party_inpers_resp.'<br>'.check_tags(trim($resp_address)).' '.$resp_disct_nm.'<br><br>' ; 
						
						echo $res_det;
						/*echo check_tags(strtoupper(trim($query_main_result['res_name']))).'<br>'; 
						...
						if($cino && isset($query_main_result['hide_partyname']) && trim($query_main_result['hide_partyname'])!='Y'){	
							...
							$addl_respondent = $HCMAS_DB->prepare("(select CAST(party_id as INTEGER ) as one,* from civ_address_t where cino= :cino and display='Y' and type='2' ) union ( select CAST(party_id as INTEGER ) as one,* from civ_address_t_a where cino=:cino and display='Y' and type='2') order by one");
							...
							if($addl_respondent){
								...
								if($addl_respondent->rowcount()){
									...
									while($addl_respondent_result = $addl_respondent->fetch()){
										...
										$addl_respon_addrs =  $HCMAS_DB->prepare("(select address,dist_code from party_address where cino= :cino AND party_no = :res_partyno AND type = '2' order by party_no) union (select address,dist_code from party_address_a where cino= :cino AND party_no = :res_partyno AND type = '2' order by party_no)");
										...
										if($addl_respon_addrs){
											...
											if($addl_respon_addrs->rowcount()){
												...
												if($addl_respo_addrs){
													...
													$addl_respon_distr = $HCMAS_DB->prepare("select dist_name from district_t where dist_code = :resp_discode");
...
<?php						
						...
						if($cino){
							...
							$extr_main_pet_advcts  = $HCMAS_DB->prepare("select * from extra_adv_t where cino =:cino and type='1' and party_no='0' order by party_no,sr_no");
							...
							$extr_peti_adv = $HCMAS_DB->prepare("( select adv_name,party_no,create_modify from civ_address_t where cino = :cino and type = '1' and display='Y' order by party_no ) union ( select adv_name,party_no,create_modify from civ_address_t_a where cino = :cino and type = '1' and display='Y' order by party_no ) ");
								...
								if($extr_peti_adv->rowcount()){
									...
									while($extr_peti_adv_result = $extr_peti_adv->fetch()){
										...
										if(trim($extr_peti_adv_result['adv_name'])){
											...
											$extr_peti_advcts = $HCMAS_DB->prepare("select * from extra_adv_t where cino = :cino and type ='1' and party_no =:ext_advpartyno order by party_no,sr_no");
...
<?php					
						...
						if($cino){
							...
							$extr_main_resp_advcts  = $HCMAS_DB->prepare("select * from extra_adv_t where cino = :cino and type='2' and party_no='0' order by party_no,sr_no");
							...
							$extr_respo_adv = $HCMAS_DB->prepare("( select adv_name,party_no,create_modify from civ_address_t where cino = :cino and type = '2' and display ='Y' order by party_no ) union ( select adv_name,party_no,create_modify from civ_address_t_a where cino = :cino and type = '2' and display ='Y' order by party_no )");
							...
							if($extr_respo_adv){
								...
								if($extr_respo_adv->rowcount()){
									while($extr_respo_adv_result = $extr_respo_adv->fetch()){
										...
										$extr_resp_advcts = $HCMAS_DB->prepare("select * from extra_adv_t where cino =:cino and type ='2' and party_no =:resp_advpartyno order by party_no,sr_no");
...
<?php						
						...
						if($query_main_result['c_subject']){
							$subject = $HCMAS_DB->prepare("select subject_name,subject_code from subject_master where subject_code =:subjct_cd");
								//var_dump($subject);
							...
							if($subject){
								...
								if($subject->rowcount()){
									...
									if(isset($query_main_result['cs_subject']) && !empty($query_main_result['cs_subject'])){
										...
										$subject_1 = $HCMAS_DB->prepare("SELECT subnature1_desc FROM subnature1_t where nature_cd =:subjct_cd and subnature1_cd= :cssub_ject1 ");
										//var_dump($subject_1);
										if($subject_1){
									...
									if(isset($query_main_result['css_subject']) && !empty($query_main_result['css_subject'])){
										...
										$subject_2 = $HCMAS_DB->prepare("SELECT subnature2_desc FROM subnature2_t where nature_cd = :subjct_cd and subnature1_cd= :cssub_ject1 and subnature2_cd= :cssub_ject2 ");
										//var_dump($subject_2);
										if($subject_2){
									...
									if(isset($query_main_result['csss_subject']) && !empty($query_main_result['css_subject'])){
										...
										$subject_3 = $HCMAS_DB->prepare("SELECT subnature3_desc FROM subnature3_t where nature_cd = :subjct_cd and subnature1_cd= :cssub_ject1 and subnature2_cd= :cssub_ject2 and subnature3_cd = :cssub_ject3  ");
										//var_dump($subject_3);
										if($subject_3){
...
<?php						
							...
							if($cino){
								...
								$case_dist = $HCMAS_DB->prepare("select case_dist_code from case_info  where cino = :cino union select case_dist_code from case_info_a  where cino = :cino ");
								if($case_dist){
									...
									if($case_dist->rowcount()){
										...
										if(trim($case_dist_result['case_dist_code'])){
											...
											$disct_case = $HCMAS_DB->prepare("select dist_name from  district_t where dist_code = :case_distcode ");
											if($disct_case){
...
<?php						
						...
						if(trim($query_main_result['dt_regis'])>'2018-09-25'){
							...
							$nature_writ = $HCMAS_DB->prepare("select nature_desc from nature_t where nature_cd = :case_naturecd and case_type_cd = :register_casetyp ");
...
<?php
							if(trim($query_main_result['cino'])){
								...
								$ntce_cont = '';
								$cse_notc_count = '';
								$notice_status =  $CIS_DB->prepare("select served_date from tn_notice_servestatus where cino = :cino ");
								//$notice_status =  $CIS_DB->query("select served_date from tn_notice_servestatus where cino ='HCMA013663802018' ");
						...
						if(trim($query_main_result['judge_code'])){
							...
							foreach($jcode as $value){
								if($value){
									...
									$corm_judcode = $value ;
									$qry_judg_nm = $HCMAS_DB->prepare("select judge_name from judge_name_t where judge_code = :corm_judcode");
							...
							if($resCauselist){
								...
								if($resCauselist->rowCount()){
									...
									if($causelist_res_lt){
										...
										$resCauselis = $HCMAS_DB->prepare("SELECT cause_list_type FROM cause_list_period where cause_list_type_id = :causelist_reslt ");
...
<?php
						...
						if(trim($query_main_result['bench_type'])) {
							...
							$res_benchtype = $HCMAS_DB->prepare("SELECT * FROM bench_type where bench_type_code= :maincase_bench");
							...
							if($item_no){
								...
								if($item_no->rowCount()){
									...
									if($sr_no_itm){
										...
										$item_no = $HCMAS_DB->prepare("(select sr_no,clink_code,causelist_date from cause_list where case_no = :item_caseno and elimination !='Y'   union select sr_no,clink_code,causelist_date from cause_list_a where case_no = :item_caseno and elimination !='Y' order by causelist_date desc limit 1 )");
										//var_dump($item_no);
										if($item_no){
...
?>						
				...
				<tr> 
					...
					<td>
<?php
							$return_case_dt = $HCMAS_DB->prepare("select objprepare_dt,srno from objection_history where cino = :cino union  select objprepare_dt ,srno from objection_history_a where cino = :cino order by srno desc limit 1");
...
<?php
						$last_presnt_dt ='';
						$last_represnt_dt = $CIS_DB->prepare("select obj_represent_dt from tn_objection_history_records where cino = :cino order by obj_srno desc limit 1");
...
<?php 
			...
			if($cino){
				...
				$countrs = $CIS_DB->prepare("select * from tn_documentregister where cino = :cino  and docu_type='21'");
...
?>
							...
							<table class="table_caseno_search">
								...
								<tr  class="hidden-sm"  >
									<td class='' > Sl. No. </td>
									<td class=''  > Counter Filed on </td>
									<td class=''  > Filed by </td>
									<td class='' > Updated on </td>
								</tr>
<?php
						   $si=1;
...
<?php
			if($cino){	
				...
				$tlcourt = $HCMAS_DB->prepare("select lower_court_code,lower_court,lower_court_dec_dt,lower_trial  from trial_lower_court where cino =:cino order by lower_trial");
				//var_dump($tlcourt);
				//$tlcourt = $HCMAS_DB->query("select lower_court_code,lower_court from trial_lower_court where cino ='HCMA010495962016' order by lower_trial");
				...
				if($tlcourt){
					...
					if($tlcourt->rowCount()){
							...
							echo "<th class='hidden-lg hidden-md'>Lower case Number</th><td>";
								...
								if(trim($tlcourt_result['lower_court'])){
									...
									if((int)$tlcourt_result['lower_trial']==3){
										...
										$qry_app_high_court = $HCMAS_DB->prepare("SELECT * FROM civil_t where case_no= :lco_urtno UNION SELECT * FROM civil_t_a where case_no= :lco_urtno ");
										...
										$qryLowCaseType = $HCMAS_DB->prepare("SELECT type_name FROM case_type_t where case_type=:lcourt_casetype");
										...
										$qryLowCaseType = $HCMAS_DB->prepare("SELECT type_name FROM lcase_type_t where lcase_type=:lcourt_casetype ");
										//var_dump($qryLowCaseType);
										if($qryLowCaseType){
											$qryLowCaseType->execute(array(':lcourt_casetype'=>$lcourt_casetype));
											if($qryLowCaseType->rowCount()){
							...
							echo "<th class='hidden-lg hidden-md'>Lower Court Name and District</th><td>";
								...
								if((int)$tlcourt_result['lower_trial']==3){
									...
									if(trim($tlcourt_result['lower_court_code'])){
										...
										$lwcort_distrct ='';
										$lwcourt_distict ='';
										$lwcount_nme = $HCMAS_DB->prepare("select lower_court_name,dist_code from lcourt_t where lower_court_code = :lowwr_code ");
										...
										if($lwcount_nme){
											if($lwcount_nme->rowCount()){
												...
												if(trim($lwcort_distrct)){
													$lwcort_dist = $HCMAS_DB->prepare("select dist_name from district_t where dist_code = :lwcort_distrct");
						...
						echo "<tr align='center' ><td colspan='4' >No records </td></tr>";
					}
				}
			}
...
<?php
	...
	if(trim($query_main_result['main_case_no'])!='' || trim($query_main_result['main_matter_cino'])!=''){
		...
		if($case_maincasenumbr!=''){
			...
			$main_cass =  $HCMAS_DB->prepare("(select * from civil_t where case_no = :maincase_numb ) union  (select * from civil_t_a where case_no = :maincase_numb ) order by fil_no,fil_year");
			$main_cass->execute(array(':maincase_numb'=>$case_maincasenumbr));
			
		} else if($case_maincino!=''){
			
			$main_cass =  $HCMAS_DB->prepare("(select * from civil_t where cino = :main_matr_cino) union  (select * from civil_t_a where cino = :main_matr_cino ) order by fil_no,fil_year");
		...
		if($main_cass){
			if($main_cass->rowcount()){
				...
				while($main_cass_result = $main_cass->fetch()){
					...
					if($main_cas_type){
						$main_css_nme = '';
						$main_css_name = $HCMAS_DB->prepare("select type_name,full_form from case_type_t WHERE case_type = :fullfrm_maincse ");
	...
	//echo "				<td class='result_td'>".$main_css_nme.".".$main_cass_result['fil_no']."/".$main_cass_result['fil_year']."&nbsp;&nbsp;(Filing Stage)"."</td>";
				...
				$main_cse_prayer = $HCMAS_DB->prepare("select prayer_cd,prayer_type from case_info as a join prayer_t as b on a.prayer_cd = b.prayercode where cino =:pryer_minncase union select prayer_cd,prayer_type from case_info_a as a join prayer_t as b on a.prayer_cd = b.prayercode where cino = :pryer_minncase");
	...
	if(trim($query_main_result['main_case_no'])==''){
		...
		if(trim($query_main_result['case_no'])){
			...
			$wmp_cass = $HCMAS_DB->prepare("(select * from civil_t where main_case_no = :mnn_caseno and cino!= :cino ) union  (select * from civil_t_a where main_case_no = :mnn_caseno and cino!=:cino ) order by fil_no,fil_year");
			
			$wmp_cass->execute(array(':cino'=>$cino,':mnn_caseno'=>$query_main_result['case_no']));
			
		} else if(trim($query_main_result['main_matter_cino'])){
			 
			$wmp_cass = $HCMAS_DB->prepare("(select * from civil_t where main_matter_cino = :mnm_cino and cino!= :cino ) union  (select * from civil_t_a where main_matter_cino = :mnm_cino and cino!= :cino) order by fil_no,fil_year");
			
			$wmp_cass->execute(array(':cino'=>$cino,':mnm_cino'=>$query_main_result['main_matter_cino']));
			
		} else if(trim($query_main_result['cino'])){
			 
			$wmp_cass = $HCMAS_DB->prepare("(select * from civil_t where main_matter_cino = :cino and cino!= :cino ) union  (select * from civil_t_a where main_matter_cino = :cino and cino!=:cino) order by fil_no,fil_year");
		...
		$wmp_cass = $HCMAS_DB->prepare("(select * from civil_t where main_case_no = :mmncaseno and cino!= :cino ) union  (select * from civil_t_a where main_case_no = :mmncaseno and cino!=:cino) order by fil_no,fil_year");
	...
	echo "			<tr class='heding_td' >";
					...
					if($wmp_cases_type){
						$wmp_cses_name = $HCMAS_DB->prepare("select type_name,full_form from case_type_t WHERE case_type = :wmp_csety_pe");
	...
	//echo "				<td class='result_td'>".$wmp_cses_typ_name.".".$wmp_cases_result['fil_no']."/".$wmp_cases_result['fil_year']."&nbsp;&nbsp;(Filing Stage)"."</td>";
					...
					//prayer details	
					$wmp_case_pryer = '';
					$wmp_cse_prayer = $HCMAS_DB->prepare("select prayer_cd,prayer_type from case_info as a join prayer_t as b on a.prayer_cd = b.prayercode where cino = :wmpcses_cino union select prayer_cd,prayer_type from case_info_a as a join prayer_t as b on a.prayer_cd = b.prayercode where cino = :wmpcses_cino");
					//var_dump($main_cse_prayer);
	...
	//ia filing starting
			if(trim($query_main_result['cino'])){
				//$ia_filng = $HCMAS_DB->query("(select * from ia_filing where cino = '".trim($query_main_result['cino'])."' order by date_of_ia_registration,ia_regno) union (select * from ia_filing_a where cino = '".trim($query_main_result['cino'])."' order by date_of_ia_registration,ia_regno) ");
				
				$all_rows_ia = '';
				$ia_filng = $HCMAS_DB->prepare("(select * from ia_filing where cino = :cino ) union (select * from ia_filing_a where cino = :cino) order by date_of_ia_registration,ia_regno ");
				//var_dump($ia_filng);
...
?>			
			...
			<table class="table_caseno_search">
				...
				<tbody>
<?php
			$conncted_cases = $HCMAS_DB->prepare("select * from connected_t a join civil_t b on a.cino = b.cino where a.linkcino = :cino and a.cino!= :cino and (b.main_matter_cino ='' or b.main_matter_cino is null or main_case_no ='' or main_case_no  is null )  union select * from connected_t a join civil_t_a b on a.cino = b.cino where a.linkcino = :cino and a.cino!= :cino and  (b.main_matter_cino ='' or b.main_matter_cino is null ) and ( main_case_no ='' or main_case_no is null ) order by reg_year,reg_no ");
				...
				if($conncted_cases->rowCount()){
					...
					while($conncted_cases_result =$conncted_cases->fetch()){
						...
						if(trim($conncted_cases_result['regcase_type']) and trim($conncted_cases_result['reg_no']) and trim($conncted_cases_result['reg_year']))
						{
							...
							$conectd_cse_type = $HCMAS_DB->prepare("select type_name from case_type_t WHERE case_type =:connctedcse_tyyp");
							if($conectd_cse_type){
							...
							if(trim($conncted_cases_result['filcase_type']) and trim($conncted_cases_result['fil_no']) and trim($conncted_cases_result['fil_year']))
							{
								...
								$fil_cse_tp = $HCMAS_DB->prepare("select type_name from case_type_t WHERE case_type =:connctad_filcastyp");
								if($fil_cse_tp){
...
?>
			...
			<table class="table_caseno_search">
				...
				<tbody>
            		...
            		<td class="text-center"> Relief Details </td>
            		...
            		<td class="text-center result_td"> 
            			<?php 
            				echo $releif_res ='';
            				/*$releif = $HCMAS_DB->query("select relief_offense  from case_info where cino ='".$query_main_result['cino']."' union select relief_offense  from case_info_a where cino ='".$query_main_result['cino']."'");
            				if($releif){
            					if($releif->rowcount()){
...
<?php
				...
				if(trim($query_main_result['branch_id'])=='4'){
					...
					if(trim($query_main_result['cino'])){
						$prayer = $HCMAS_DB->prepare("select relief_offense  from case_info where cino = :cino union select relief_offense  from case_info_a where cino = :cino");
						if($prayer){
...
<?php
    ...
    if(trim($query_main_result['cino'])){
		...
		if(!$regnoo){
			...
			$filing_hist = $HCMAS_DB->query("select * from daily_proc_filing where cino='".$query_main_result['cino']."' order by todays_date NULLS FIRST");
			...
			if($filing_hist){
				...
				if($filing_hist->rowcount()){
					...
					echo "<tr> <td align='center' colspan ='7'> <b>SR Stage History of Hearing</b> </td></tr>";
					...
					while($filing_hist_res = $filing_hist->fetch()){
						...
						foreach($jcode1 as $value) {
							if($value){
								$qry_judg_nm1 = $HCMAS_DB->query("select judge_name from judge_name_t where judge_code ='".$value."'");
						...
						echo '<tr>';
								...
								if($item_no){
									...
									if($item_no->rowCount()){
										...
										if($sr_no_itm){
											...
											$item_no = $HCMAS_DB->prepare("select sr_no,clink_code from cause_list where case_no = :clink_itm and causelist_date = :hofh_todaysdate and elimination !='Y'  union select sr_no,clink_code from cause_list_a where case_no = '".$clink_itm."' and causelist_date = :hofh_todaysdate and elimination !='Y'   ");
											//var_dump($item_no);
											if($item_no){
							...
							if($purpse_code){
								$purpse_nme = $HCMAS_DB->query("select purpose_name from purpose_t where purpose_code ='".$filing_hist_res['purpose_code']."' ");  
								//var_dump($purpse_nme);
								if($purpse_nme){
		...
		if($query_histry_herng){
			...
			if($query_histry_herng->rowcount()){
				...
				while ($result_histry_herng = $query_histry_herng->fetch()){
					...
					foreach($jcode1 as $value) {
						if($value){
							$qry_judg_nm1 = $HCMAS_DB->prepare("select judge_name from judge_name_t where judge_code = :hofh_jcode");
							//var_dump("select judge_name from judge_name_t where judge_code ='".$value."'");
					...
					if($purpse_code){
						$purpse_nme = $HCMAS_DB->prepare("select purpose_name from purpose_t where purpose_code = :hofh_purcod ");  
						//var_dump($purpse_nme);
						if($purpse_nme){
					...
					echo "<tr>
								...
								if($item_no){
									...
									if($item_no->rowCount()){
										...
										if($sr_no_itm){
											...
											$item_no = $HCMAS_DB->prepare("select sr_no,clink_code from cause_list where case_no = :hofh_clink and causelist_date = :hofh_toddaydate and elimination !='Y'  union select sr_no,clink_code from cause_list_a where case_no = :hofh_clink and causelist_date = :hofh_toddaydate and elimination !='Y'   ");
											//var_dump($item_no);
											if($item_no){
						...
						if(trim($result_histry_herng['adjcode'])){
							...
							$adj_cdd = $HCMAS_DB->prepare("select adjname from adjcode_t where adjcode = :hofh_ajcode");
							if($adj_cdd){
				...
				if($query_main_result['date_of_decision'] ==''){
					...
					echo "<tr>";
						$last_row =  $HCMAS_DB->prepare("select * from disposal_proc where cino = :cino");
						//var_dump($last_row);
						if($last_row){
							...
							if($last_row->rowCount()){
								...
								foreach($jcode2 as $value1) {
									$qry_judg_nm2 = $HCMAS_DB->prepare("select judge_name from judge_name_t where judge_code = :hofh_disp_jcode");
								...
								//var_dump($disp_name);
								echo "<th class='hidden-lg hidden-md'>Judge</th><td>".rtrim(strtoupper($judge_name2),', ')."</td>";
								$sr_no_itm ='';
								$clink_itm ='';
								$item_noo_date ='';
								if($cino){
									 
									$item_no = $HCMAS_DB->prepare("select sr_no,clink_code from cause_list where cino = :cino and causelist_type !='1009' and causelist_sr_no != 0 and elimination !='Y' and causelist_date < DATE(NOW())+1 order by causelist_date desc limit 1");
									//var_dump($item_no);
									if($item_no){
										$item_no->execute(array(':cino'=>$cino));
										if($item_no->rowCount()){
											...
											if($sr_no_itm){
												...
												$item_no = $HCMAS_DB->prepare("select sr_no,clink_code from cause_list where case_no = :dips_hofh_lkitem  order by causelist_date desc limit 1 ");
												if($item_no){
								...
								echo "<th class='hidden-lg hidden-md'>Item no.</th><td>".check_tags($item_noo_date)."</td>";
								echo "<th class='hidden-lg hidden-md'>Business On Date</th><td>".implode('/',array_reverse(explode('-',check_tags($last_row_result['todays_date']))))."</td> ";
								
								
								echo "<th class='hidden-lg hidden-md'>Business</th><td>".check_tags($last_row_result['order_remark'])."</td>";
								...
								if(trim($query_main_result['disp_nature'])){
									...
									$disp_natre = $HCMAS_DB->prepare("select disp_name from disp_type_t where disp_type = :hofh_dispnt and disp_type!=0");
									if($disp_natre){
										...
										echo "<th class='hidden-lg hidden-md'>Business</th><td>".$disp_name."</td>";
								} else {
									echo "<th class='hidden-lg hidden-md'>Purpose of hearing</th><td>".$purpose."</td> ";
								} 
				...
				if($srrecds){
					...
					echo "<tr> <td class='text-center' align='center' colspan ='7'> No records </td><tr>";
				}
			} //row count
		} //query not working
	} //cino emtpy
...
<?php		
	...
	if($cino){	
		...
		$documnt_reg = $CIS_DB->prepare("select * from tn_documentregister where cino= :cino");
			...
			while($documnt_reg_result = $documnt_reg->fetch()){
				echo "<tr>";
					...
					if($documnt_reg_result['docu_type']==0){
						...
						$doc_typ_nm = $HCMAS_DB->prepare("select docu_name from docu_type_t where docu_type= :doct_typ_ee ");
						if($doc_typ_nm){
			...
			if($caveat_no_all){
				...
				foreach($caveat_no_all as $value){
					...
					if(trim($value)){
						...
						$caveat_qry = $HCMAS_DB->query("SELECT * FROM caveat_t where caveat_no = '".trim($value)."' union SELECT * FROM caveat_t_a where caveat_no ='".trim($value)."'");
						 
						//$caveat_qry = $HCMAS_DB->query("SELECT * FROM caveat_t where cino = 'HCMA01C000022018' union SELECT * FROM caveat_t_a where cino = 'HCMA01C000022018'");
						//var_dump($caveat_qry);
...
<?php
		...
		if($case_name  and $case_no and $case_year){
			...
			$noticess_qry = $CIS_DB->prepare("SELECT * FROM tn_notice_servestatus where regcase_type = :casenam_e and reg_no = :case_no_o and reg_year = :case_ye_ar  and casenotype = 'F' and despatch_entry_date IS NOT NULL  and display='Y'  order by party_no,type,process_id");
			...
			<table class="table_caseno_search">
				<thead  class="hidden-sm"   >
					<tr >
						<td> Sl.No. </td>
						<td> Party </td>
						<td> Dispatch Date(Notice) </td>
						<td> Seat </td>
						<td> Mode of Dispatch </td>
						<td> Dispatch Date(CS)  </td>
						<td> Status  </td>
						<td> Served Date/(Return Date & Reason)  </td>
					</tr>
				</thead>
				<tbody>
				...
				while($notices_result = $noticess_qry->fetch()){
					...
					echo "	<th class='hidden-lg hidden-md'>Seat</th><td>";
						$notce_sectn = '';
						 if(trim($notices_result['despatch_section'])){
							 ...
							 $sectn_qry = $HCMAS_DB->prepare("SELECT * FROM section_t WHERE sectioncode = :dispct_sectn ");
							 if($sectn_qry){
					...
					echo "	<th class='hidden-lg hidden-md'>Mode of Dispatch</th><td>";
						...
						if(check_tags($notices_result['bailiff_id'])){
							...
							$notce_balif = $CIS_DB->prepare("SELECT bailiffname FROM tn_baliff_t where bailiffid = :balidf_di ");
					...
					echo "	<th class='hidden-lg hidden-md'>Served Date/(Return Date & Reason)</th><td>";
						if($serv_dt){
							...
							if(trim($notices_result['reason_code'])){
								$resn_notcee ='';
								$notc_resn_code = $HCMAS_DB->prepare("SELECT * FROM process_unsuccessful_t  where unsuccessful_id = :reason_cdoe ");
								if($notc_resn_code){
...
<?php		
		if($cino){
			$sno=0;
			$order_qry = $HCMAS_DB->prepare("SELECT * FROM order_details WHERE cino=:cinno UNION SELECT * FROM order_details_a WHERE cino=:cinno order by order_dt, order_no");
			/*$order_qry = $HCMAS_DB->prepare("select * from (
				...
				if($order_qry->rowcount()){
					...
					//	$file_name = '192.168.1.36/hc_cis_mas/orders/'.$ord_yr[0].'/'.$order_caseno."_".$order_no.".pdf";
						...
						if(!trim($order_jocode)){
							...
							$query = $HCMAS_DB->query("select a.judge_code,a.judge_name as judge_name,a.desg_code,a.short_judge_name from Judge_name_t as a,judge_t as b where a.display='Y' and a.judge_code=b.judge_code and b.court_no='".$order_court_no."' and from_dt IS NOT NULL order by b.judge_priority");
							...
							if($orjc1)
							{
								...
								//var_dump($order_jocodes);
									  
									// GET MULTI JUDGE					  
									//echo "select * from judge_name_t where judge_code in(".rtrim($order_jocode,",").") and display='Y'";
								if($order_jocodes){
									
									$query = $HCMAS_DB->query("select * from judge_name_t where judge_code in(".rtrim($order_jocodes,",").") and display='Y' order by judge_priority asc");
								...
								$query = $HCMAS_DB->query("select * from judge_name_t where judge_code='".$order_jocode."' and display='Y' order by judge_priority asc"); 
							} 
						} //else ending
						...
						if($file_name!='404') // CHECK WHETHER ORDER COPY IS EXISTS OR NOT
						{
...
?>	
				...
				<tr class="<?php echo $file_name;?>" id="<?php echo $order_cino;?>">
		          ...
		          <th class='hidden-lg hidden-md'>Order Copy</th>
...
<?php						
						...
						echo '<h3 style="color:red;">Please enter appropriate input value.</h3>';
					}//
				} else {
					echo '<h3 style="color:red;">Please check the madatory fields are filled</h3>';
				}//emtpy input
			} else {
