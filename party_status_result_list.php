			...
			$reg_case_type = $getdata->getcasetype($query_main_result['filcase_type']);
			 $case_details_valu = $reg_case_type.'.'.$query_main_result['fil_no'].'/'.$query_main_result['fil_year'].'<br>(filing stage)';
				...
				//echo "<br/>its if: select a.judge_code,a.judge_name as judge_name,a.desg_code,a.short_judge_name from Judge_name_t as a,judge_t as b where a.display='Y' and a.judge_code=b.judge_code and b.court_no='".$order_court_no."' and from_dt IS NOT NULL order by b.judge_priority";
					$query = $HCMAS_DB->query("select a.judge_code,a.judge_name as judge_name,a.desg_code,a.short_judge_name from Judge_name_t as a,judge_t as b where a.display='Y' and a.judge_code=b.judge_code and b.court_no='".$court_no."' and from_dt IS NOT NULL order by b.judge_priority");
				...
				else 
				{
					...
					if(($orjc1))
					{
						...
						$query=$HCMAS_DB->query("select * from judge_name_t where judge_code in(".rtrim($jocode,",").") and display='Y' order by judge_priority  ");
						...
						$query = $HCMAS_DB->query("select * from judge_name_t where judge_code='".$jocode."' and display='Y' "); 
					}
				} //else 	
				...
				<tr  id="<?php echo $cino;?>">
				   ...
				   <td style="font-weight:bold;text-align:left;"><?php echo $judges;?></td>
				    ...
				    <form method="POST" id="<?php echo 'pdf_capt_sub_'.$sno; ?>" action="case_status_party_result.php"  target="_blank" >
