...
<table id="example1" class="display responsive" cellspacing="0" width="100%">
    <thead>
      ...
      <th style="width:40%" align="left">Type</th>
	    <th style="width:20%">Click Here</th>
	...
	$cur_year=date('Y');
	$curr_dt=date('Y-m-d');
	 $qry = $DB_con->query("select * from mhc_transfer where display='Y' AND notification_year='".$cur_year."' order by transfer_order desc,transfer_date DESC");
while($row = $qry->fetch())
{
		...
		/*if($row['transfer_cadre']=='DJ')
					...
					if(trim(trim($row['transfer_type']))=='T')
					{
					...
					if($row['icon']=='PDF')
		{
        $icon='<img width="30" height="30" src="admin/images/pdf.png"/ alt="PDF file that opens in new window" width="20" height="20"/>';
		}
		else if($row['icon']=='XLS')
		{
		$icon='<img width="30" height="30" src="admin/images/xlsx.png"/ alt="PDF file that opens in new window" width="20" height="20"/>';
		}
		else if($row['icon']=='IMG')
		{
		$icon='<img width="30" height="30" src="admin/images/img.png"/ alt="PDF file that opens in new window" width="20" height="20"/>';
		}
      ...
      <td style="width:40%" align="left"><?php echo $transfer_type; ?>&nbsp;<?php echo $new; ?></td>
	  ...
	  </form><!--<a href="admin/view_pdf.php?pdf_id=<?php echo $row['transfer_id'] ?>&page=T" alt="PDF file that opens in new window" target="_blank" onclick="alert('External Website that opens in a new window')" ></a>--></td>
