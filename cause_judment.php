...
?>
<style>
#bench1,#bench2,#bench3,#bench4,#RegCase_type {
    max-width: 100%;
    width: 100%;
}
						   ...
						   <h2 class="post-title" align="center">THE JUDGMENTS INFORMATION SYSTEM [ Case No Search ]</h2>				
								<div id="comments" class="themeform">	
									...
									<p class="comment-form-author">
										  ...
										  <select type="text" name="bench1" id="bench1" class="form-control"  autocomplete="off" required onchange="setBench('case_no')" >
											<option value="" >Choose Bench</option>     
											   <?php
											   $dqry=$DB_con->prepare("select name,value from drop_down where display='Y' and col_nme='jbench' and page_id='1000' order by order_id");
											   $dqry->execute();
						...
						<div class="tab-pane" id="resp-tab2">
						  <h2 class="post-title" align="center">THE JUDGMENTS INFORMATION SYSTEM [Party Name Search ]</h2>				
								<div id="comments" class="themeform">	
									...
									<p class="comment-form-author">
										  <label for="cause_list_bench">Bench<span class="required">*</span></label>
										  <select type="text" name="bench2" id="bench2" class="form-control"  autocomplete="off" required onchange="setBench('party')" >
											<option value="" >Choose Bench</option>     
											   <?php
											   $dqry=$DB_con->prepare("select name,value from drop_down where display='Y' and col_nme='jbench' and page_id='1000' order by order_id");
											   $dqry->execute();
											   while ($dqry_row = $dqry->fetch()) 
				{
						...
						<div class="tab-pane" id="resp-tab3">
								...
								<div id="comments" class="themeform">	
									...
									<p class="comment-form-author">
										  <label for="cause_list_bench">Bench<span class="required">*</span></label>
										  <select type="text" name="bench3" id="bench3" class="form-control"  autocomplete="off" required onchange="setBench('judge')" >
											<option value="" >Choose Bench</option>     
											   <?php
											   $dqry=$DB_con->prepare("select name,value from drop_down where display='Y' and col_nme='jbench' and page_id='1000' order by order_id");
											   $dqry->execute();
...
function extractLast( term ) {
   return split( term ).pop();
				...
				<p class="comment-form-author">
										  <label for="cause_list_bench">Bench<span class="required">*</span></label>
										  <select type="text" name="bench4" id="bench4" class="form-control"  autocomplete="off" required onchange="setBench('order_dt')" >
											<option value="" >Choose Bench</option>     
											   <?php
											   $dqry=$DB_con->prepare("select name,value from drop_down where display='Y' and col_nme='jbench' and page_id='1000' order by order_id");
											   $dqry->execute();
...
$("#casenosearch").validate({
	...
	messages: {		
		bench1: {
			required: "Please select bench type"
	...
	submitHandler: function(form) {
		...
		var posting = $.post( "cause_judment_action1.php", { bench:$("#bench1").val(),RegCase_type:$("#RegCase_type").val(),RegCase_no:$("#RegCase_no").val(),RegCase_year:$("#RegCase_year").val(),caseno_captcha:$("#caseno_captcha").val()});
		posting.done(function( data ) {						
		    //$("#orddate_captcha").val("");
			$("#caseno_search_result").html(data);
...
$("#partysearch").validate({
	...
	submitHandler: function(form) {
		...
		var posting = $.post( "cause_judment_action1.php", { bench:$("#bench2").val(),party_name:$("#party_name").val(),from_date:$("#from_date").val(),to_date:$("#to_date").val(),party_captcha:$("#party_captcha").val(),party_type:party_type});
		posting.done(function( data ) {						
		    //$("#orddate_captcha").val("");
...
$("#judgesearch").validate({
	...
	submitHandler: function(form) {
		...
		var posting = $.post( "cause_judment_action1.php", { bench:$("#bench3").val(),judge_name:$("#judge_name").val(),jud_from_date:$("#jud_from_date").val(),jud_to_date:$("#jud_to_date").val(),judge_captcha:$("#judge_captcha").val()});
		posting.done(function( data ) {						
		    //$("#orddate_captcha").val("");
...
$("#order_datesearch").validate({
	...
	submitHandler: function(form) {
		...
		var posting = $.post( "cause_judment_action1.php", { bench:$("#bench4").val(),order_date:$("#order_date").val(),orddate_captcha:$("#orddate_captcha").val()});
		posting.done(function( data ) {						
		    //$("#orddate_captcha").val("");
