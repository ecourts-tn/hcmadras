...
if($ann_fun->ann_update1($edit_id,$ann_lable,$ann_title,$ann_pdf,$ann_size,$date_up,$exp_dt,$ann_lan,$ann_order,$display,$new,$mhc_user,$icon_img,$page_id,$ip,$log_fun,$ann_link,$elink))
{
	unset($error);
$ann_fun->redirect("anno_management.php?joined1&CheckString=".$chkstr."&".md5('page_id')."=".$page_id); 
}
}
}						
}
...
{
	if($ann_fun->ann_update($edit_id,$ann_lable,$ann_title,$ann_size,$date_up,$exp_dt,$ann_lan,$ann_order,$display,$new,$mhc_user,$icon_img,$page_id,$ip,$log_fun,$ann_link,$elink))
{
	$ann_fun->redirect("anno_management.php?joined1&CheckString=".$chkstr."&".md5('page_id')."=".$page_id); 
						}
}
		 	 ...
		 	 if(isset($_GET['edit_id']))
{
 ...
 $stmt = $DB_con->prepare("SELECT  * FROM announcement where an_id=:id");
 $stmt->execute(array(':id' => $edit));
 $editRow=$stmt->FETCH(PDO::FETCH_ASSOC);

 if(is_null($editRow['an_update_date']))
 {
	 $upload_date='';
 }
 else
 {
	 $upload_date=date('d-m-Y',strtotime($editRow['an_update_date']));
 }
 if(is_null($editRow['an_archive']))
 {
	 $an_archive='';
 }
 else
 {
	 $an_archive=date('d-m-Y',strtotime($editRow['an_archive']));
 }
}
 ...
 ?>
        <div class="main-content-wrap sidenav-open d-flex flex-column">
            ...
            <div class="main-content">
                ...
                <div class="row mb-4">
                    ...
                    <div class="col-md-12 mb-4">
                        <div class="card">
                            <div class="card-body">
                                <div class="card-title">Announcements Edit Form</div>
								<?php
				if(isset($error))
            {
            ...
            {
                 ?>
				 <div class="alert alert-card alert-success" role="alert"><strong class="text-capitalize">New Announcements Added Successfully</strong>
                            <button class="close" type="button" data-dismiss="alert" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                        </div>
            ...
            {
                 ?>
				 <div class="alert alert-card alert-success" role="alert"><strong class="text-capitalize">Announcements Updated Successfully</strong>
                            <button class="close" type="button" data-dismiss="alert" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                        </div>
            ...
            {
                 ?>
				 <div class="alert alert-card alert-success" role="alert"><strong class="text-capitalize">Announcements Deleted Successfully </strong>
                            <button class="close" type="button" data-dismiss="alert" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                        </div>
			...
			?>
								...
								<input type="hidden" name="edit_id" value="<?php  if(isset($_GET['edit_id'])){ print($editRow['an_id']); }?>" >
                                    <div class="form-row">
									 <div class="col-md-2 form-group mb-3">
                                            <label for="validationTooltip01">Announcements Lable</label>
                                            <select class="form-control" id="ann_lable" name="ann_lable" type="text" placeholder="Announcements name"  required="required" aria-describedby="validationTooltipFirstPrepend" onKeyPress="return ValidateAlpha(event);" autocomplete="off" onchange='checkdata()' >
								...
								if($d_qry->rowCount() > 0){
											while($m=$d_qry->fetch()){
												if(isset($_GET['edit_id'])){ if($editRow['an_label']==$m['value']){
                                        ...
                                        <div class="col-md-6 form-group mb-3">
                                            ...
                                            <input class="form-control" id="ann_title" name="ann_title" type="text" placeholder="Announcements name"  required="required" aria-describedby="validationTooltipFirstPrepend" autocomplete="off"  value="<?php  if(isset($_GET['edit_id'])){ print(str_replace('"',"'",$editRow['an_text'])); }?>" />
                                            <div class="invalid-tooltip">
										...
										<div id='up_fld'  <?php if($editRow['an_label']!='link') {?> style='display:block'<?php }else {?> style='display:none'<?php }?>>
                                            <label for="validationTooltip02">Uploads Announcements</label>
											 <div class="custom-file">
                                        <input class="custom-file-label" id="ann_pdf" name="ann_pdf" type="file" aria-describedby="inputGroupFileAddon01" onchange="return ValidateFileUpload()">
											...
											<div id='link_fld' <?php if($editRow['an_label']=='link') {?> style='display:block'<?php }else {?> style='display:none'<?php }?>>
											 <label for="validationTooltip02">Enter Link</label>
											 <div>
                                        <input class="form-control" id="ann_link" name="ann_link"  type="text" aria-describedby="inputGroupFileAddon01" placeholder="Enter Link" value="<?php  if(isset($_GET['edit_id'])){ print($editRow['ann_link']); }?>"/>
										</div>
                                         <div class="invalid-tooltip">
										...
										<div class="col-md-2 form-group mb-3" id='link_fld1' <?php if($editRow['an_label']=='link') {?> style='display:block'<?php }else {?> style='display:none'<?php }?>>
                                            ...
                                            <div class="input-group">
                                              ...
                                              <option value="Y"<?php  if(isset($_GET['edit_id'])){ if($editRow['external_link']=='Y'){ echo "SELECTED"; }}?>>Yes</option>
											   <option value="N"<?php  if(isset($_GET['edit_id'])){ if($editRow['external_link']=='N'){ echo "SELECTED"; }}?>>No</option>
                                            </select>
                                            </div>
                                        </div>
										<div class="col-md-2 form-group mb-3">
								...
								if($d_qry->rowCount() > 0){
											while($m=$d_qry->fetch()){
												if(isset($_GET['edit_id'])){ if($editRow['an_icon_img']==$m['value']){
												echo "<option value='".$m['value']."' selected>".$m['name']."</option>";}
												else{
												echo "<option value='".$m['value']."' >".$m['name']."</option>";}}
								}}?>
										...
										<div class="col-md-2 form-group mb-3">
                                         ...
                                         <div class="input-group">
                                                ...
                                                <input class="form-control datepicker" id="upload_date" name="upload_date" type="text" placeholder="Announcements on" aria-describedby="validationTooltipPasswordPrepend"  autocomplete="off"  value="<?php  if(isset($_GET['edit_id'])){ print($upload_date); }?>"/>
                                                <div class="invalid-tooltip">
										...
										<div class="col-md-2 form-group mb-3">
                                         ...
                                         <div class="input-group">
                                                <div class="input-group-prepend"><span class="input-group-text" id="validationTooltipPasswordPrepend">@</span></div>
                                                <input class="form-control datepicker" id="exp_date" name="exp_date" type="text" placeholder="Announcements on" aria-describedby="validationTooltipPasswordPrepend"  autocomplete="off"  value="<?php  if(isset($_GET['edit_id'])){ print($an_archive); }?>"/>
                                                <div class="invalid-tooltip">
                                        ...
                                        <div class="col-md-2 form-group mb-3">
                                            <label for="validationTooltipUsername">Announcements Language</label>
                                             <select class="form-control" id="ann_lan" name="ann_lan" type="text" placeholder="Announcements Language"   autocomplete="off" >
											  <option value="" disabled>Choose</option>
                                              <option value="English"<?php  if(isset($_GET['edit_id'])){ if($editRow['an_pdf_lanuage']=='English'){ echo "SELECTED"; }}?>>English</option>
											   <option value="Tamil"<?php  if(isset($_GET['edit_id'])){ if($editRow['an_pdf_lanuage']=='Tamil'){ echo "SELECTED"; }}?>>Tamil</option>
										...
										<div class="col-md-1 form-group mb-3">
                                            ...
                                            <div class="input-group">
                                                ...
                                                <input class="form-control Number" id="ann_order" name="ann_order" type="text" placeholder="Announcements Order" aria-describedby="validationTooltipPasswordPrepend"  autocomplete="off" value="<?php  if(isset($_GET['edit_id'])){ print($editRow['an_order']); }?>"/>
                                                <div class="invalid-tooltip">
										...
										<div class="col-md-1 form-group mb-3">
                                            ...
                                            <div class="input-group">
                                                ...
                                                <option value="">Choose</option>
                                              <option value="Y"<?php  if(isset($_GET['edit_id'])){ if($editRow['an_new_icon']=='Y'){ echo "SELECTED"; }}?>>Yes</option>
											   <option value="N"<?php  if(isset($_GET['edit_id'])){ if($editRow['an_new_icon']=='N'){ echo "SELECTED"; }}?>>No</option>
                                            </select>
                                            </div>
                                        </div>
										<div class="col-md-2 form-group mb-3">
			...
			function Validator(theform)
{
   ...
   else if(chkbadchar(theform.ann_size.value)==false )
   {
