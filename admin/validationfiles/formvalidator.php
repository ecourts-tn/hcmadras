...
define("E_VAL_BADCHR_CHECK_FAILED","Please provide a valid input for %s");
define("E_VAL_BADSYM_CHECK_FAILED","Please provide a valid input for %s");
...
class FormValidator 
{
	...
	function FltStr($SStr){
 $Zz=$SStr;
   $Zz=str_replace("|","",str_replace("*/","",str_replace("/*","",str_replace("'","",str_replace("/**/","",$Zz)))));
   $Zz=str_replace("0x","",str_replace("%3","",str_replace("%2","",str_replace("%1","",str_replace("/..","",$Zz)))));
   $Zz=str_replace("()","",str_replace(";","",str_replace("$","",str_replace(">","",str_replace("<","",str_replace("--","",$Zz))))));
   $Zz=str_replace("=","",str_replace("\\","",str_replace("%","",str_replace("*","",str_replace("**","",$Zz)))));
   $Zz=str_replace("select","",str_replace("drop","",str_replace("insert","",str_replace("delete","",str_replace("xp_","",$Zz)))));
   $Zz=str_replace("script","",str_replace("alert","",str_replace("update","",str_replace("exec(","",str_replace("ascii(","",$Zz)))));
return $Zz;
}
...
if(checkdate($y, $m, $d)) {
 $bret = true;
  ...
  return $bret;	 
	...
	function validate_req($input_value, &$default_error_message,$variable_name)
	{
	  $bret = true;
      	if(!isset($input_value) ||
			strlen($input_value) <=0 || empty($input_value))
		{
			$bret=false;
	...
	$badch = array('SELECT','INSERT','DELETE','DROP','ALTER','CHANGE','MODIFY','UNION','CHAR','CONVERT','CAST','--','<','>','#', '*', '^','~','`','!','%','&','+','=','|',':',';','?','{','}','\'','--','"');	
	...
	//function chkbadchar()
	function chkpdffiles($pdf_files){ 
	$allowed_types = array ('application/pdf');
$fileInfo = finfo_open(FILEINFO_MIME_TYPE);
$detected_type = finfo_file( $fileInfo, $pdf_files);
if ( !in_array($detected_type, $allowed_types) ) {
	...
	function ValidateCommand($command,$command_value,$input_value,&$default_error_message,$variable_name,$formvariables)
	{
		...
		switch($command)
		{
			...
			case 'badsym':
						{
							$bret= $this->test_datatype($input_value,"[^0-9a-zA-Z_@$]");
							if(false == $bret)
							{
								$default_error_message = sprintf(E_VAL_BADSYM_CHECK_FAILED,$variable_name);
							}
							break;
						}
			case 'badchar':
			{
							...
							{
								$bret= $this->chkbadchar($input_value); 
								if(false == $bret)
								{
									$default_error_message = E_VAL_BADCHR_CHECK_FAILED;
								}
							}
							break;
						}			
