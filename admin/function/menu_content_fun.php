...
//User Register
	 ...
	 public function menu_content_insert($title,$desc,$display,$mhc_user,$bd22,$page_id,$ip,$log_fun)
    {
       try
       {
			   $sql2= "INSERT INTO mhc_menu_content(title,m_desc,display) VALUES('".$title."','".$desc."','".$display."')returning h_id";
			      ...
			      if($stmt)
			{
				...
				$record_id=$row -> h_id;
				$message='Added Menu content Details';
				$action="INSERT INTO mhc_menu_content(title,m_desc,display) VALUES('$title','$desc','$display')";
	...
	public function menu_content_update($edit_id,$title,$desc,$display,$mhc_user,$bd22,$page_id,$ip,$log_fun)
    {
       try
       {
			    ...
			    $sql2= "UPDATE mhc_menu_content SET title='$title',m_desc='$desc',display='$display' WHERE h_id ='$edit_id'";
			   $stmt = pg_query($bd22,$sql2);
			        if($stmt)
			{
				
				$record_id=$edit_id;
				$message='Update menu content Details';
				$action="UPDATE mhc_menu_content SET title='$title',m_desc='$desc',display='$display' WHERE h_id ='$edit_id'";
 ...
 public function menu_content_Delete($del_id,$mhc_user,$page_id,$ip,$log_fun)
    {
       ...
       try
       {
 		  ...
 		  $stmt =  $this->db->prepare("DELETE FROM mhc_menu_content WHERE h_id=:del_id");
          $stmt->bindValue(':del_id', $del_id, PDO::PARAM_STR);
          if($stmt->execute())
		  {
			  $record_id=$edit_id;
				$message='Delete Home Details';
				$action="DELETE FROM mhc_menu_content WHERE h_id=$del_id";
	  ...
	  catch(PDOException $e)
       {
		...
		public function menu_content_data($doc_show_page_str,$chkstr,$mhc_user,$page_id,$ip,$log_fun)
	{
		...
		try {
				...
				$GetData ='';
				 ...
				 $stmt = $this->db->prepare("SELECT * FROM mhc_menu_content where title in (".$doc_show_page_str.") ORDER BY h_id ASC");
				 $stmt->execute();				 
				$sno=1;
				while ($row = $stmt->fetch()) {
					   ...
					   $enctype_id=base64_encode($row['h_id']);
						...
						<td>'.$sno.'</td>
                          <td>'.$row['title'].'</td>                            
                          <td>'.html_entity_decode($row['m_desc']).'</td>
                          <td>'.$display.'</td>						  
                          <td class="text-right">
                            <a href="menu_content_management_edit.php?edit_id='.$enctype_id.'&CheckString='.$chkstr.'&'.md5('page_id').'='.$page_id.'" class="btn btn-info m-1"><i class="i-Pen-2"></i></a>
 ...
 public function menu_content_photo_reg($photo_title,$img_thmp,$display,$file_type,$mhc_user,$bd22,$page_id,$ip,$log_fun)
    {
       try
       {
	...
	public function menu_content_photo_data($chkstr,$bd22,$page_id)
	{
		...
		try {
				...
				$GetData ='';
					 ...
					 $stmt = $this->db->prepare("select * from home_gallery ORDER BY h_gallery_id ASC");
				 $stmt->execute();				 
				$sno=1;
				while ($row = $stmt->fetch()) {
					...
					$img='<img src="view_image.php?img_id='.trim(base64_encode($row['h_gallery_id'])).'&page='.base64_encode('H').'" width="50" height="50"/>';
						...
						else
						{
							...
							$img='<a  href="view_pdf.php?pdf_id='.base64_encode($row['h_gallery_id']).'&page='.base64_encode('H').'" class="btn btn-primary" target="_blank">Document View</a>';
						}
...
public function menu_content_photo_data1($chkstr,$bd22,$page_id)
	{
...
public function menu_content_photo_update($edit_id,$photo_title,$img_thmp,$display,$mhc_user,$file_type,$bd22,$page_id,$ip,$log_fun)
    {
       try
       {
