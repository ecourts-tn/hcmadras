 ...
 else if(!empty($display) && $validator->chkbadchar($display) == false)
 {
  $error = "Please enter valid Display ";
   ...
   else if(empty($reg_pri))
   {
      ...
      $error = "Enter your Registrar Priority !";
   }
 else if(!empty($reg_pri) && $validator->chkbadchar($reg_pri) == false)
 {
  $error = "Please enter valid Registrar Priority ";
 }
...
if ( !in_array($detected_type, $allowed_types)&& $_FILES['photo']['name'] ) {
	...
	else
	{
		...
		{
			$binary_bar = file_get_contents("images/Registrar.jpg");
			$dp_image = pg_escape_bytea($binary_bar);
...
if($reg_fun->reg_update($edit_id,$reg_name,$reg_app_date,$reg_rel_date,$designation,$cont_no,$fax_no,$place,$display,$dp_image,$reg_pri,$pre_name,$mhc_user,$page_id,$ip,$log_fun,$def_chk))
{
	unset($error);
							$reg_fun->redirect("reg_management.php?joined1&CheckString=".$chkstr."&".md5('page_id')."=".$page_id); 
						} 
...
{
	if($reg_fun->reg_update1($edit_id,$reg_name,$reg_app_date,$reg_rel_date,$designation,$cont_no,$fax_no,$place,$display,$reg_pri,$pre_name,$mhc_user,$page_id,$ip,$log_fun,$def_chk))
{
	unset($error);
							$reg_fun->redirect("reg_management.php?joined1&CheckString=".$chkstr."&".md5('page_id')."=".$page_id); 
						} 
	 ...
	 if(isset($_GET['edit_id']))
{
 ...
 $stmt = $DB_con->prepare("SELECT  * FROM registrars where reg_id=:id");
 $stmt->execute(array(':id' => $edit));
 $editRow=$stmt->FETCH(PDO::FETCH_ASSOC);

 if(is_null($editRow['reg_app']))
 {
	 $reg_app='';
 }
 else
 {
	 $reg_app=date('d-m-Y',strtotime($editRow['reg_app']));
 }
 if(is_null($editRow['reg_rel']))
 {
	 $reg_rel='';
 }
 else
 {
	 $reg_rel=date('d-m-Y',strtotime($editRow['reg_rel']));
 }
}
        ...
        <div class="main-content-wrap sidenav-open d-flex flex-column">
            ...
            <div class="main-content">
                ...
                <div class="row mb-4">
                    ...
                    <div class="col-md-12 mb-4">
                        <div class="card">
                            <div class="card-body">
                                <div class="card-title">Registrar Update Form</div>
								<?php
				if(isset($error))
            {
			...
			?>
                                <form class="needs-validation" novalidate="novalidate" action="" method="POST" enctype="multipart/form-data" onSubmit="return Validator(this)">
								 <input type="hidden" name="edit_id" value="<?php  if(isset($_GET['edit_id'])){ print($editRow['reg_id']); }?>" >
                                    <div class="form-row">
									<div class="col-md-2 form-group mb-1">
                                            <label for="validationTooltip01">Prefix Name</label>
                                            <input class="form-control" id="pre_name" name="pre_name" type="text" placeholder="Prefix name"  required="required" aria-describedby="validationTooltipFirstPrepend" onKeyPress="return ValidateAlpha(event);" autocomplete="off" value="<?php  if(isset($_GET['edit_id'])){ print($editRow['reg_prefix']); }?>" />
                                        ...
                                        <div class="col-md-3 form-group mb-3">
                                            ...
                                            <input class="form-control" id="reg_name" name="reg_name" type="text" placeholder="Registrar name"  required="required" aria-describedby="validationTooltipFirstPrepend" onKeyPress="return ValidateAlpha(event);" autocomplete="off" value="<?php  if(isset($_GET['edit_id'])){ print($editRow['reg_name']); }?>"/>
                                            <div class="invalid-tooltip">
                                               Enter Your Registrar Name
                                        ...
                                        <div class="col-md-3 form-group mb-3">
											...
											<option value="">Choose</option>
											 <?php
											 $select_qry = $DB_con->query("select sno,desig from officer_designation where display='Y' order by desig asc");
while($row = $select_qry->fetch())
{
	if($editRow['reg_desig']==$row['sno'])
	{
		echo "<option value=".$row['sno']." SELECTED>".$row['desig']." </option>";
	}
	else
	{
...
?>
										...
										<div class="col-md-4 form-group mb-3">
											...
											<?php
											$reg_contact_no=explode(',',$editRow['reg_contact_no']);
											$cont_count=count($reg_contact_no);
				$i=1;
				foreach($reg_contact_no as $cont_no)
				{
					...
					if($i==$cont_count)
					{
						$cbt='btn-success btn-add';
						$cic='+';
					}
					else
					{
						$cbt='btn-danger btn-remove';
						$cic='-';
					}
					?>
					<div class="form-group multiple-form-group input-group">
					<input type="text" name="cont_no[]" class="form-control Number" placeholder="Enter Contact Number" value="<?php  if(isset($_GET['edit_id'])){ print($cont_no); }?>" maxlength="12"    autocomplete="off">
                        <span class="input-group-btn">
						
                            <button type="button" class="btn <?php echo $cbt ?>"><?php echo $cic ?></button>
                        </span>
											</div>
				...
				$i++;
                                            ...
                                            <!--<input class="form-control Number" id="cont_no" name="cont_no" type="text" placeholder="Enter Contact Number" maxlength="8"  required="required"  autocomplete="off" value="<?php  if(isset($_GET['edit_id'])){ print($editRow['reg_contact_no']); }?>"/>-->
                                            <div class="invalid-tooltip">
                                        ...
                                        <div class="col-md-4 form-group mb-3">
                                            ...
                                            <div class="input-group">
                                                ...
                                                <input class="form-control datepicker" id="reg_app_date" name="reg_app_date" type="text" placeholder="Registrar  Appointed on" aria-describedby="validationTooltipUsernamePrepend" required="required"  autocomplete="off" value="<?php  if(isset($_GET['edit_id'])){ print($reg_app); }?>"/>
                                                <div class="invalid-tooltip">
										...
										<div class="col-md-4 form-group mb-3">
                                            ...
                                            <div class="input-group">
                                                ...
                                                <input class="form-control datepicker" id="reg_rel_date" name="reg_rel_date" type="text" placeholder="Registrar Relieved on" aria-describedby="validationTooltipPasswordPrepend"  autocomplete="off" value="<?php  if(isset($_GET['edit_id'])){ print($reg_rel); }?>"/>
                                                <div class="invalid-tooltip">
										...
										<div class="col-md-4 form-group mb-3">
                                            ...
                                            <div class="input-group">
                                                <?php
												$reg_fax_no=explode(',',$editRow['reg_fax_no']);
												$fax_count=count($reg_fax_no);
				$j=1;
				foreach($reg_fax_no as $fax_no)
				{
					if($j==$fax_count)
					{
						$fbt='btn-success btn-add';
						$fic='+';
					}
					else
					{
						$fbt='btn-danger btn-remove';
						$fic='-';
					}
					?>
					<div class="form-group multiple-form-group input-group">
					<input type="text" name="fax_no[]" class="form-control Number" placeholder="Enter Contact Number" value="<?php  if(isset($_GET['edit_id'])){ print($fax_no); }?>" maxlength="12"    autocomplete="off">
                        <span class="input-group-btn">
						
                            <button type="button" class="btn <?php echo $fbt ?>"><?php echo $fic ?></button>
                        </span>
											</div>
					<?php
					$j++;
												...
												?>
                                               <!-- <input class="form-control Number" id="fax_no" name="fax_no" type="text" placeholder="Fax No" aria-describedby="validationTooltipemailidPrepend" required="required"  autocomplete="off" maxlength="8" value="<?php  if(isset($_GET['edit_id'])){ print($editRow['reg_fax_no']); }?>"/>-->
                                                <div class="invalid-tooltip">
                                    ...
                                    <div class="form-row">
                                        <div class="col-md-3 form-group mb-3">
											...
											<img src="view_image.php?img_id=<?php echo base64_encode($editRow['reg_id'])?>&page=<?php echo base64_encode('R')?>" width="50" height="50"  />
                                          
                                    <div class="custom-file">
                                        <input class="custom-file-label" id="photo" name="photo" type="file" aria-describedby="inputGroupFileAddon01" value="" onchange="return ValidateFileUpload()">
								...
								if($d_qry->rowCount() > 0){
											while($m=$d_qry->fetch()){
												if(isset($_GET['edit_id'])){ if($editRow['reg_place']==$m['value']){
									...
									<div class="form-row">
                                        <div class="col-md-4 form-group mb-3">
                                            <label for="validationTooltip03">Registrar Priority</label>
                                            <input class="form-control Number" id="reg_pri" 
											name="reg_pri" type="text" placeholder="Registrar Priority" required="required"  autocomplete="off" maxlength='2' value="<?php  if(isset($_GET['edit_id'])){ print($editRow['reg_pl_sen']); }?>"/>
                                            <div class="invalid-tooltip">
                                                Please provide a valid Registrar Priority.
