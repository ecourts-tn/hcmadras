...
<div class="pad group">
<h2 class="post-title" align="center">Former Hon'ble Chief Justice</h2><br>
 
 <table id="table_id2"  >
    <thead>
        <tr>
            <th> Former Hon'ble Chief Justices</th>            
        </tr>
    </thead>
    <tbody>
	...
	$sql1 = $DB_con->query("select * from judges where j_display='Y'  and  j_page='FC' order by cj_tdt desc");
...
$profile=$row_jud['j_profile'];
	...
	$cj_fdt=$row_jud['cj_fdt'];
	$cj_tdt=$row_jud['cj_tdt'];
	if(strlen(rtrim(ltrim($cj_fdt," ")," "))==4)
		$period=$cj_fdt." - ".$cj_tdt;
	else
	{
		$t_cj_fdt=explode("-",$cj_fdt);
		$t_cj_tdt=explode("-",$cj_tdt);
		$period=$t_cj_fdt[2]."-".$t_cj_fdt[1]."-".$t_cj_fdt[0]." to ".$t_cj_tdt[2]."-".$t_cj_tdt[1]."-".$t_cj_tdt[0];
	}
	if($j_type=='CJ')
	{
   ...
   data-content="<div><img src='admin/view_image.php?img_id=<?php echo base64_encode($row_jud['j_id'])?>&page=<?php echo base64_encode('J'); ?>'  width='250' height='250' align='center'></div> <p><?php echo nl2br($row_jud['j_profile']);?></p><br><button type='button' class='btn btn-primary' data-toggle='popover' style='float:center'>Close</button>"><img style="cursor: pointer;" width="75" height="75" src="admin/view_image.php?img_id=<?php echo base64_encode($row_jud['j_id'])?>&page_id=<?php echo base64_encode('PJ'); ?>&page=<?php echo base64_encode('J'); ?>" title="Click to Enlarge" alt="<?php echo $hon.$prefi.".Justice ".$name.$fjudge; ?>" id="judge_thumb" /></a>
			...
			<div class="col-md-8">
			  <div class="judge_container">
				...
				<?php if(rtrim(" ",ltrim(" ",$profile))){ ?>
                <br><font style="cursor: pointer;" class="profont" data-toggle="collapse" data-target="#<?php echo $row_jud['j_id'];?>">Profile</font>	
				<?php } ?>	
				</div>
