...
$jud_code=$_POST['jud_cd'];

include_once('validationfiles/formvalidator.php'); 
$validator = new FormValidator(); 

$securimage = new Securimage(array('namespace' => 'judgename'));
$valid = $securimage->check($_POST['judgename_captcha']);

$security_code = filter_input(INPUT_POST, 'captcha', FILTER_SANITIZE_STRING);
		/*Validate Session and User Captcha Values*/
		if ($valid){ 		
			
if($_POST['submit'] && $_POST['cause_list_dt']!='' && $_POST['jud_cd']!='')
{
...
else if(empty($jud_code))
   {
        echo '<br><h3 style="color:red;text-align:center;">Enter your Judge Code !</h3>';
     
   }
 else if(!empty($jud_code) && $validator->chkbadchar($jud_code) == false)
 {
	  echo '<br><h3 style="color:red;text-align:center;">Please enter valid Judge Code</h3>';

 }
 else if(!empty($jud_code) && $validator->test_datatype($jud_code,"[^0-9]") == false)
 {
  echo '<br><h3 style="color:red;text-align:center;">Please enter valid  Numeric  Judge Code</h3>';
 } 
 else
 {
...
$judge_code='';
$jud_bench_qry=$MDU_HCMAS_DB->query("SELECT judge_code FROM judge_t WHERE court_no ='".$jud_bench."' ");
 while($jud_bench_row=$jud_bench_qry->fetch())
 {
	 $judge_code =$jud_bench_row['judge_code'];
	 $Judge_name_val_temp .= $getdata->GetJudgeName($judge_code).", ";
 }
  $Judge_name_val=rtrim($Judge_name_val_temp,', ');
...
?>
				...
				<h6 class="post-title" style='color:red' align="center"><b>Cause List For <?php echo $causelist_dt?></b></h6>
				<h6 class="post-title" style='color:red' align="center">Search Results For : <?php echo $Judge_name_val;?></h6>
...
<table id="judge" class="display responsive nowrap" style="width:100%">
