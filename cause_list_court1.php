...
{ 	
    ...
    if($_POST['submit'] && $_POST['cause_list_dt']!='' && $_POST['court_no']!='' && $_POST['court_no']!='0' && $_POST['bench']!='')
{
	$bench=$_POST['bench'];
	if($bench=='MHC'){
...
$causelist_dt=$_POST['cause_list_dt'];
 ...
 $cause_list_title=$DB_option->query("SELECT causelist_sr_no,for_bench_id,causelist_id,causelist_date,court_id FROM causelist_title WHERE cause_list_status='P' AND trim(court_id)='".$court_no."' AND causelist_date ='".$causelist_date."' order by causelist_id ");
		...
		$judgrows++;
			...
			while($link_cause_list_row=$link_cause_list_qry->fetch())	
			{
				...
				if($link_cause_list_row['on_filing']=='Y')
				{
					...
					$link_case_details="SR No. ".$link_fil_type.'.'.$link_RegCase_no.'/'.$link_RegCase_year;
					 ...
					 $link_civil_t_a_qry = $DB_option->query("SELECT date_filing_disp,disp_nature FROM civil_t_a where cino='".$link_cino."' and date_filing_disp='$causelist_date'  ORDER BY date_filing_disp DESC");
						 ...
						 $link_todays_date=date('d-M-Y',strtotime($disp_c_nat['date_filing_disp']));
						  ...
						  {
							  $link_resSrCaseType = $DB_option->query("SELECT todays_date,order_remark,next_date FROM daily_proc_filing where cino='".$link_cino."' and todays_date='$causelist_date'  ORDER BY todays_date DESC");
				...
				else
				{
					...
					$link_daily_pro = $DB_option->query("SELECT adjcode, srno,todays_date,order_remark,next_date FROM daily_proc where cino='".$link_cino."' and todays_date='$causelist_date'  UNION SELECT adjcode, srno,todays_date,order_remark,next_date FROM daily_proc_a where cino='".$link_cino."' and todays_date='$causelist_date' ORDER BY todays_date DESC");
					...
					{
						$link_disposal_qry = $DB_option->query("SELECT order_remark FROM disposal_proc where cino='".$link_cino."' and todays_date='$causelist_date'  ORDER BY todays_date DESC");
							if($link_disposal_qry->rowcount()>0)
						  {
							  $link_disposal_row = $link_disposal_qry->fetch();
							   
						$link_dis_pos_case=$DB_option->prepare("SELECT disp_nature,date_of_decision FROM civil_t_a  where cino=:cino and date_of_decision='$causelist_date'  ORDER BY date_of_decision DESC");
						...
						$link_todays_date='';	
						  ...
						  {
							   $link_civil_t_query1 = $DB_option->query("SELECT disp_nature,date_of_decision FROM civil_t_a where cino='".$link_cino."' and date_of_decision='$causelist_date'  ORDER BY date_of_decision DESC");
						  ...
						  {
							  $link_restore_qry=$DB_option->prepare("SELECT remark,next_date,todays_date FROM restorerevoke  where cino=:cino  AND datetype='rev' and todays_date='$causelist_date'  ORDER BY todays_date DESC");
			...
			else
			{
				...
				if($cause_list_row['on_filing']=='Y')
				{
					...
					$civil_t_a_qry = $DB_option->query("SELECT date_filing_disp,disp_nature FROM civil_t_a where cino='".$cino."' and date_filing_disp='$causelist_date' ORDER BY date_filing_disp DESC");
						 ...
						 $todays_date='';
						  ...
						  {
							  $resSrCaseType = $DB_option->query("SELECT todays_date,order_remark,next_date FROM daily_proc_filing where cino='".$link_cino."' and todays_date='$causelist_date' ORDER BY todays_date DESC");
				...
				else
				{
					...
					//echo "SELECT adjcode, srno FROM daily_proc where cino='".$cino."' and todays_date='".$causelist_date."' UNION SELECT adjcode, srno FROM daily_proc_a where cino='".$cino."' and todays_date='".$causelist_date."' ORDER BY srno DESC";
					 $daily_pro = $DB_option->query("SELECT adjcode, srno,todays_date,order_remark,next_date FROM daily_proc where cino='".$cino."' and todays_date='$causelist_date'  UNION SELECT adjcode, srno,todays_date,order_remark,next_date FROM daily_proc_a where cino='".$cino."' and todays_date='$causelist_date'  ORDER BY todays_date DESC");
					...
					{
						$disposal_qry = $DB_option->query("SELECT order_remark FROM disposal_proc where cino='".$cino."' and todays_date='$causelist_date'  ORDER BY todays_date DESC");
							if($disposal_qry->rowcount()>0)
						  {
							  $disposal_row = $disposal_qry->fetch();
							   
						$dis_pos_case=$DB_option->prepare("SELECT disp_nature,date_of_decision FROM civil_t_a  where cino=:cino and date_of_decision='$causelist_date'  ORDER BY date_of_decision DESC");
						...
						$todays_date='';	
						  ...
						  {
							   $civil_t_query1 = $DB_option->query("SELECT disp_nature,date_of_decision FROM civil_t_a where cino='".$cino."' and date_of_decision='$causelist_date'  ORDER BY date_of_decision DESC");
						  ...
						  {
							  $restore_qry=$DB_option->prepare("SELECT remark,next_date,todays_date FROM restorerevoke  where cino=:cino  AND datetype='rev' and todays_date='$causelist_date'  ORDER BY todays_date DESC");
