...
<div class="pad group">
		...
		<div class="card" align="center"><i class="fa fa-video-camera"></i>&nbsp;&nbsp;MADRAS HIGH COURT VIDEO CONFERENCING DETAILS (<?php echo date("d-m-Y");?>)</div>	
...
<table   width="100%">
  ...
  //error_log("SELECT bench_id FROM tn_causelist WHERE causelist_date = '".$cur_date."' AND court_det = 'MHC' AND published='Y'  GROUP BY bench_id");
   $qry1 = $CIS_DB->query("SELECT bench_id FROM tn_causelist WHERE causelist_date = '".$cur_date."' AND court_det = 'MHC' AND published='Y'  GROUP BY bench_id");
   		if($qry1->rowCount() > 0) {
...
$ben_id=rtrim($bench_id,',');
  ...
  $i=1;
	 $qry = $CIS_DB->query("select * from tv_vc_det where bench_id IN (".$ben_id.") and court_det='MHC' and to_date >='".$cur_date."' and  from_date <='".$cur_date."' order by cause_no ASC");
	 if($qry->rowCount() > 0) {
while($row = $qry->fetch())
{
 ...
 if($cur_date=='2024-10-08'){
		   $i=1;
	 $qry = $CIS_DB->query("select * from tv_vc_det where bench_id IN ('16448') and court_det='MHC' and to_date >='".$cur_date."' and  from_date <='".$cur_date."' order by cause_no ASC");
	 if($qry->rowCount() > 0) {
while($row = $qry->fetch())
{
