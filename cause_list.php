...
<link rel="stylesheet" href="css/jquery-ui.css">
	...
	<div class="container" id="page">
				...
				<div class="main-inner group">
				   <div class="content">
					...
					<div class="post-title"  align="center">
						<div class="card" align="center"><i class=""></i>&nbsp;&nbsp;MADRAS HIGH COURT CAUSE LIST</div>
						</div>
								...
								<div id="comments" class="themeform">	
									...
									<form  method="post" id="cause_list_court" class="commentform comment-form">	
										...
										<p class="comment-form-author">
										  <label for="cause_list_bench">Bench<span class="required">*</span></label>
										  <select type="text" name="bench1" id="bench1" class="form-control"  autocomplete="off" required onchange="setBench('court_no')" >
											<option value="" >Choose Bench</option>     
											   <?php
											   $dqry=$DB_con->prepare("select name,value from drop_down where display='Y' and col_nme='bench' and page_id='0' order by order_id");
											   $dqry->execute();
											   while ($dqry_row = $dqry->fetch()) 
				{
					
						echo '<option value="'.$dqry_row['value'].'">'.$dqry_row['name'].'</option>';
					}
											   ?>
											</select>
						...
						<div class="tab-pane" id="resp-tab2">
						  <h2 class="post-title" align="center">CAUSE LIST [Hon'ble Judge Name Search ]</h2>				
								<div id="comments" class="themeform">	
									...
									<p class="comment-form-author">
										  <label for="cause_list_bench">Bench<span class="required">*</span></label>
										  <select type="text" name="bench2" id="bench2" class="form-control"  autocomplete="off" required onchange="setBench('jud_name')" >
											<option value="" >Choose Bench</option>     
											   <?php
											   $dqry=$DB_con->prepare("select name,value from drop_down where display='Y' and col_nme='bench' and page_id='0' order by order_id");
											   $dqry->execute();
        ...
        <?php  //$stmt2 = $HCMAS_DB->prepare("SELECT judge_code,judge_name,display FROM judge_name_t where display='Y' ORDER by judge_code");
		/*$stmt2 = $HCMAS_DB->prepare("select ben.court_no as court_no,UPPER(STRING_AGG(judge_name, ', ')) as judge_name from judge_name_t nam inner join  (select 
court_no,judge_code from judge_t where court_no in (SELECT distinct for_bench_id FROM causelist_title WHERE cause_list_status='P' AND 
 causelist_date ='2023-04-28')) ben on nam.judge_code =ben.judge_code group by ben.court_no order by judge_name");
				 $stmt2->execute();				 
				
				while ($row2 = $stmt2->fetch()) 
				{
					
						echo '<option value="'.$row2['court_no'].'">'.$row2['judge_name'].'</option>';
					}
					*/
					?>
						...
						<div class="tab-pane" id="resp-tab3">
								...
								<p class="comment-form-author">
																		  <label for="cause_list_bench">Bench<span class="required">*</span></label>
																		  <select type="text" name="bench3" id="bench3" class="form-control"  autocomplete="off" required onchange="setBench('enrol_no')" >
																			<option value="" >Choose Bench</option>     
																			   <?php
																			   $dqry=$DB_con->prepare("select name,value from drop_down where display='Y' and col_nme='bench' and page_id='0' order by order_id");
																			   $dqry->execute();
						...
						<div class="tab-pane" id="resp-tab4">
						    ...
						    <h2 class="post-title" align="center">CAUSE LIST [Advocate Name Search ]</h2>				
								<div id="comments" class="themeform">	
									...
									<p class="comment-form-author">
										  <label for="cause_list_bench">Bench<span class="required">*</span></label>
										  <select type="text" name="bench4" id="bench4" class="form-control"  autocomplete="off" required onchange="setBench('adv_name')" >
											<option value="" >Choose Bench</option>     
											   <?php
											   $dqry=$DB_con->prepare("select name,value from drop_down where display='Y' and col_nme='bench' and page_id='0' order by order_id");
											   $dqry->execute();
											   while ($dqry_row = $dqry->fetch()) 
				{
						...
						<div class="tab-pane" id="resp-tab5">
						   ...
						   <p class="comment-form-author">
										  <label for="cause_list_bench">Bench<span class="required">*</span></label>
										  <select type="text" name="bench5" id="bench5" class="form-control"  autocomplete="off" required onchange="setBench('case_no')" >
											<option value="" >Choose Bench</option>     
											   <?php
											   $dqry=$DB_con->prepare("select name,value from drop_down where display='Y' and col_nme='bench' and page_id='0' order by order_id");
											   $dqry->execute();
						...
						echo '<option value="'.$dqry_row['value'].'">'.$dqry_row['name'].'</option>';
                                        ...
                                        <p class="comment-form-author">
										  ...
										  <option value="">Choose Case Type</option>
										   <?php  /*$stmt = $HCMAS_DB->prepare("SELECT case_type,type_name,full_form FROM case_type_t where display='Y' ORDER BY case_type");
				 $stmt->execute();				 
				...
				while ($row = $stmt->fetch()) 
				{
					...
					?>
										...
										<p class="comment-form-author">
										   ...
										   <input type="text" class="form-control" id="case_no" name="case_no" placeholder="Case No" onkeypress="return isNumber(event)" autocomplete="off" required>
										</p>
										...
										<p class="comment-form-author">
										   ...
										   <input type="text" class="form-control" maxlength="4" id="case_year" name="case_year" placeholder="Year" onkeypress="return isNumber(event)" autocomplete="off" required>
										</p>
						    ...
						    <h2 class="post-title" align="center">CAUSE LIST [Party Name Search ]</h2>				
								<div id="comments" class="themeform">	
									...
									<form action="#" method="post" id="partyname_list_case" class="commentform comment-form">
										<p class="comment-form-author">
										  <label for="cause_list_bench">Bench<span class="required">*</span></label>
										  <select type="text" name="bench6" id="bench6" class="form-control"  autocomplete="off" required onchange="setBench('party_name')" >
											<option value="" >Choose Bench</option>     
											   <?php
											   $dqry=$DB_con->prepare("select name,value from drop_down where display='Y' and col_nme='bench' and page_id='0' order by order_id");
											   $dqry->execute();
...
$(function(){
	...
	$("#cause_list_court").validate({
		...
		submitHandler: function(form) {
			...
			var bench1=$("#bench1").val();
			
			var posting = $.post( "cause_list_court1.php", { cause_list_dt:$("#cause_list_dt1").val(), court_no :$("#court_no").val(),courtno_captcha:$("#courtno_captcha").val(),bench:bench1,submit:$("#submit").val()});
		...
		/*submitHandler: function(form) {
			...
			$("#cause_list_court_no_search").html("<div align='center'><img src='images/spinner1.gif' width='300' height='300'/></div>");
			var posting = $.post( "cause_list_court.php", { cause_list_dt:$("#cause_list_dt1").val(), court_no :$("#court_no").val(),courtno_captcha:$("#courtno_captcha").val(),submit:$("#submit").val()});
			posting.done(function( data ) {
				$("#cause_list_court_no_search").html(data);
	...
	$("#cause_list_jud").validate({
		...
		submitHandler: function(form) {
			...
			// REFRESH CAPTCHA				
			var bench2=$("#bench2").val();
			
			$("#cause_list_jud_search").html("<div align='center'><img src='images/spinner1.gif' width='300' height='300'/></div>");
			var posting = $.post( "cause_list_jud1.php", { cause_list_dt:$("#cause_list_dt2").val(), jud_cd :$("#jud_cd").val(),judgename_captcha:$("#judgename_captcha").val(),bench:bench2,submit:$("#submit").val()});
			posting.done(function( data ) {
		...
		submitHandler: function(form) {
			...
			var bench3=$("#bench3").val();
			
			var posting = $.post("cause_list_advcd1.php", { cause_list_dt:$("#cause_list_dt3").val(), enroll_id :$("#enroll_id").val(),aorno_captcha:$("#aorno_captcha").val(),bench:bench3,submit:$("#submit").val()});
			posting.done(function( data ) {
	...
	$("#cause_list_adv_name").validate({
		...
		submitHandler: function(form) {
			...
			// REFRESH CAPTCHA
			var bench4=$("#bench4").val();
			if(bench4=='MHC')
				var file4="cause_list_adv_name.php";
			else
				var file4="cause_list_adv_name_mdu.php";
			var posting = $.post( "cause_list_adv_name1.php", { cause_list_dt:$("#cause_list_dt4").val(), adv_name :$("#adv_name").val(),advname_captcha:$("#advname_captcha").val(),bench:bench4,submit:$("#submit").val()});
			posting.done(function( data ) {
	...
	$("#cause_list_case").validate({
		...
		// Specify validation error messages
		messages: {
			bench5: "Please select bench",
			cause_list_dt5: "Please select the Cause List Date",
		...
		submitHandler: function(form) {
			...
			// REFRESH CAPTCHA
			var bench5=$("#bench5").val();
			
			var posting = $.post( "cause_list_case1.php", { cause_list_dt:$("#cause_list_dt5").val(), case_type :$("#case_type").val(),case_no :$("#case_no").val(),case_year :$("#case_year").val(),caseno_captcha:$("#caseno_captcha").val(),bench:bench5,submit:$("#submit").val()});
			posting.done(function( data ) {
	...
	$("#partyname_list_case").validate({
		...
		submitHandler: function(form) {
			...
			var bench6=$("#bench6").val();
			if(bench6=='MHC')
				var file6="cause_list_party_name.php";
			else
				var file6="cause_list_party_name_mdu.php";
						
			var posting = $.post( "cause_list_party_name.php", { cause_list_dt:$("#cause_list_dt6").val(), party_name :$("#party_name").val(),bench:bench6,partyname_captcha:$("#partyname_captcha").val(),submit:$("#submit").val()});
			posting.done(function( data ) {
	...
	$("#entire_list_case").validate({
		...
		submitHandler: function(form) {
			...
			var bench7=$("#bench7").val();
			if(bench7=='MHC')
				var file7="cause_list_entire.php";
			else
				var file7="cause_list_entire_mdu.php";		
			var posting = $.post( file7, { cause_list_dt:$("#cause_list_dt7").val(), entirecause_captcha:$("#entirecause_captcha").val(),submit:$("#submit").val()});
			posting.done(function( data ) {
	...
	function getcourt() {
		...
		var bench=$('#bench1').val();
		
		if(bench=='MHC')
			var action_value='getcourt';
		else
			var action_value='getmducourt';
	...
	{
		var bench=$('#bench2').val();
		
		if($("#cause_list_dt2").val()){
			$("#jud_cd").html('');
		var jud_list=$.post("get_jud_list.php",{getCauseListValue:true,bench:bench,cause_date:$("#cause_list_dt2").val()});
