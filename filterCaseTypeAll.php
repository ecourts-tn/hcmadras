...
$where = "";
 ...
 if(isset($_SESSION['MainSection']) && $_SESSION['MainSection'] != "") {
	if($_SESSION['MainSection'] == "J") {
		...
		$excJudicial = $HCMAS_DB->query("SELECT * FROM court_t WHERE court_no = '1'");
		...
		////////WRIT
		$excWrit = $HCMAS_DB->query("SELECT * FROM court_t WHERE court_no = '2'");
		...
		////////ORIGINAL
		$excOriginal = $HCMAS_DB->query("SELECT * FROM court_t WHERE court_no = '3'");
		...
		////////CRIMINAL
		$excCriminal = $HCMAS_DB->query("SELECT * FROM court_t WHERE court_no = '4'");
...
$res_case_type = $HCMAS_DB->query($qry_case_type);
