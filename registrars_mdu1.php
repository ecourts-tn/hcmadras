...
<table id="example" width="100%" class="display responsive">
 ...
 $qry_reg = $DB_con->query("SELECT reg.*,des.desig as desig FROM registrars reg inner join officer_designation  des on des.sno=CAST (reg.reg_desig AS INTEGER)  
where trim(reg.reg_place)='MDU' and reg.display='Y' order by reg.reg_pl_sen asc");
$i=1;
