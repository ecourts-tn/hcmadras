...
$bar_no=$_POST['enroll_id'];

include_once('validationfiles/formvalidator.php'); 
$validator = new FormValidator(); 
$securimage = new Securimage(array('namespace' => 'aorno'));
$valid = $securimage->check($_POST['aorno_captcha']);
		/*Validate Session and User Captcha Values*/
		if ($valid){ 		
			
if($_POST['submit'] && $_POST['cause_list_dt']!='' && $_POST['enroll_id']!='')
{
...
else if(empty($bar_no))
   {
        echo '<br><h3 style="color:red;text-align:center;">Enter your Bar Code !</h3>';
     
   }
 else if(!empty($bar_no) && $validator->chkbadchar($bar_no) == false)
 {
	  echo '<br><h3 style="color:red;text-align:center;">Please enter valid Bar Code</h3>';
...
$cnr_no='';
//var_dump("SELECT cino FROM civil_t WHERE pet_adv_cd ='".$adv_code."' OR res_adv_cd ='".$adv_code."' ");
//echo "SELECT cino FROM civil_t WHERE pet_adv_cd =".$adv_code." OR res_adv_cd =".$adv_code;
 $civ_qry=$MDU_HCMAS_DB->query("SELECT cino FROM civil_t WHERE pet_adv_cd =".$adv_code." OR res_adv_cd =".$adv_code." union
SELECT cino FROM extra_adv_t WHERE adv_code =".$adv_code );
 if($civ_qry->rowCount()){
 ...
 $bench_id ='';
//ECHO "SELECT for_bench_id FROM cause_list WHERE cino IN (".$cnr_num.") and causelist_date='".$causelist_date."' UNION SELECT for_bench_id FROM cause_list_a WHERE cino IN (".$cnr_num.") and causelist_date='".$causelist_date."' ";
 $jud_bench_qry=$MDU_HCMAS_DB->query("SELECT for_bench_id FROM cause_list WHERE cino IN (".$cnr_num.") and causelist_date='".$causelist_date."' and elimination='N' UNION SELECT for_bench_id FROM cause_list_a WHERE cino IN (".$cnr_num.") and causelist_date='".$causelist_date."' and elimination='N'");
...
?>
				...
				<h6 class="post-title" style='color:red;font-family: "Times New Roman", Times, red serif;' align="center">SEARCH RESULTS FOR : <?php echo $bar_no;?></h6>
...
<table id="advcd" class="display responsive nowrap" style="width:100%">
