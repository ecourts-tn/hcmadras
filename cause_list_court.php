...
$securimage = new Securimage(array('namespace' => 'courtno'));
$securimage->getCaptchaId();
$valid = $securimage->check($_POST['courtno_captcha']);
 
if ($valid)
{ 	
	
    if($_POST['submit'] && $_POST['cause_list_dt']!='' && $_POST['court_no']!='' & $_POST['court_no']!='0')
{
...
$causelist_dt=$_POST['cause_list_dt'];
 ...
 $cause_list_title=$HCMAS_DB->query("SELECT causelist_sr_no,for_bench_id,causelist_id,causelist_date,court_id FROM causelist_title WHERE cause_list_status='P' AND trim(court_id)='".$court_no."' AND causelist_date ='".$causelist_date."' order by causelist_id ");
 while($cause_title_row=$cause_list_title->fetch())
 {
 $bench_id .="'".$cause_title_row['for_bench_id']."',";
 }
...
?>
				...
				<h2 class="post-title" align="center">PRINCIPAL SEAT OF MADRAS HIGH COURT</h2>
				...
				<h6 class="post-title" style='color:red;font-family: "Times New Roman", Times, red serif;' align="center">SEARCH RESULTS FOR COURT HALL NO: <?php echo $court_no?></h6>
			<article class="group post-1222 page type-page status-publish hentry">
			
									
				<div class="entry" align="center">
			
<table id="court" class="display responsive nowrap" style="width:100%">
        <thead style="background: #827676; color: #fff;">
           <tr>
      <th align="left">S.No.</th>
	    <th align="left" width="18%">Bench</th>
	  <th align="left">List Type</th>
	    <th align="left">Caption</th>
      <th align="left">Case Number</th>
      <th align="left">Party Name</th>
      <th align="left">Petitioner Advocate</th>
      <th align="left" >Respondent Advocate</th>
   ...
   {
      echo '<tr><td colspan="6" style="color:#f4f7f4;font-weight: bold;background-color: #a00d0d;">Enter your Cause List Date !</td><tr>';
   
   }
 else if(!empty($causelist_date) && $validator->chkbadchar($causelist_date) == false)
 {
	  echo '<tr><td colspan="6" style="color:#f4f7f4;font-weight: bold;background-color: #a00d0d;">Please enter valid Cause List Date</td><tr>';

 }
else if(empty($court_no))
   {
        echo '<tr><td colspan="6" style="color:#f4f7f4;font-weight: bold;background-color: #a00d0d;">Enter your Court No!</td><tr>';
     
   }
 else if(!empty($court_no) && $validator->chkbadchar($court_no) == false)
 {
	  echo '<tr><td colspan="6" style="color:#f4f7f4;font-weight: bold;background-color: #a00d0d;">Please enter valid Court No</td><tr>';

 } 
 else
 {
		 $cause_list=$HCMAS_DB->query("SELECT distinct(sr_no),cino,case_no,ctype,originalsr_no,case_remark,causelist_type,causelist_period,list_from_date,list_to_date,causelist_date,for_bench_id,purpose_cd,reg_dt,filing_dt,section_id,on_filing,clink_code,elimination,causelist_sr_no,cause_list_eliminate,ia_no,ia_next_date,ia_flag,search_case,initial_status,final_status,cause_case_type,cause_reg_no,cause_reg_year,ia_case_type,bunch_code,purpose_priority,short_order,court_no FROM cause_list WHERE  causelist_date ='".$causelist_date."' AND for_bench_id IN (".$jud_bench.") AND causelist_sr_no!='0' AND  originalsr_no!='0' and elimination='N' UNION SELECT distinct(sr_no),cino,case_no,ctype,originalsr_no,case_remark,causelist_type,causelist_period,list_from_date,list_to_date,causelist_date,for_bench_id,purpose_cd,reg_dt,filing_dt,section_id,on_filing,clink_code,elimination,causelist_sr_no,cause_list_eliminate,ia_no,ia_next_date,ia_flag,search_case,initial_status,final_status,cause_case_type,cause_reg_no,cause_reg_year,ia_case_type,bunch_code,purpose_priority,short_order,court_no FROM cause_list_a WHERE  causelist_date ='".$causelist_date."' AND for_bench_id IN (".$jud_bench.") AND causelist_sr_no!='0' AND  originalsr_no!='0' and elimination='N' ORDER BY sr_no");
...
$purpose_cd=$cause_list_row['purpose_cd'];
 ...
 $query=$HCMAS_DB->query("select a.judge_code,a.judge_name as judge_name,a.desg_code,a.short_judge_name from Judge_name_t as a,judge_t as b 
	...
	while($row1=$query->fetch())
	{
		...
		$judgrows++;
					...
					//IA CASE Details 
					$ia_case_qry=$HCMAS_DB->query("SELECT appotherparty,appotheradv,againotheradv,case_no,purpose_code,case_remark FROM ia_filing WHERE case_no = '".$ia_case_no."' and ia_no='".$ia_num."' union SELECT appotherparty,appotheradv,againotheradv,case_no,purpose_code,case_remark FROM ia_filing_a WHERE case_no = '".$ia_case_no."' and ia_no='".$ia_num."'");
			...
			else
			{
				...
				$ia_extra_case='<tr>
									<td align="left"></td>
									 <td align="left" width="18%">'.$judges.'</td>
                <td align="left">'.$section.'</td>
                <td align="left">'.$purpose_name.'</td>
									<td align="left">IN </br>'.$ia_extra_case_details.'</td>
									<td align="left">'.$ia_extra_party_details.'</td>
									<td align="left">'.$extra_ia_pet_adv.'</td>
									<td align="left">'.$extra_ia_res_adv.'</td>
									<td align="left"></td>
									</tr>';
			}
			}
					...
					$ia_case_data='<tr>
									...
									<td align="left">'.$section.'</td>
									<td align="left">'.$purpose_name.'</td>
									<td align="left">'.$ia_case_datails.'</td>
									<td align="left">'.$ia_party_details.'</td>
									<td align="left">'.$ia_pet_adv.'</td>
									<td align="left">'.$ia_res_adv.'</td>
									<td align="left"></td>
									</tr>';
					...
					$link_cause='';
					 ...
					 $link_cause_list_qry=$HCMAS_DB->query("SELECT cino,cause_case_type,cause_reg_no,cause_reg_year,on_filing,case_no,causelist_type FROM cause_list WHERE causelist_date ='".$causelist_date."'  AND for_bench_id = '".$for_bench_id."'  AND causelist_type='".$causelist_id."' AND clink_code ='".$case_no."' AND  elimination='N' ORDER BY sr_no");
			...
			while($link_cause_list_row=$link_cause_list_qry->fetch())	
			{
				...
				$link_civil_qry = $HCMAS_DB->prepare("SELECT filcase_type,fil_no,fil_year,pet_name,res_name,pet_adv_cd,res_adv_cd,regcase_type,reg_no,reg_year,
				...
				if($link_cause_list_row['on_filing']=='Y')
				{
					...
					$link_case_details="SR No. ".$link_fil_type.'.'.$link_RegCase_no.'/'.$link_RegCase_year;
					 ...
					 $link_civil_t_a_qry = $HCMAS_DB->query("SELECT date_filing_disp,disp_nature FROM civil_t_a where cino='".$link_cino."' and date_filing_disp='$causelist_date'  ORDER BY date_filing_disp DESC");
						 ...
						 $link_todays_date=date('d-M-Y',strtotime($disp_c_nat['date_filing_disp']));
						  ...
						  {
							  $link_resSrCaseType = $HCMAS_DB->query("SELECT todays_date,order_remark,next_date FROM daily_proc_filing where cino='".$link_cino."' and todays_date='$causelist_date'  ORDER BY todays_date DESC");
				...
				else
				{
					...
					$link_daily_pro = $HCMAS_DB->query("SELECT adjcode, srno,todays_date,order_remark,next_date FROM daily_proc where cino='".$link_cino."' and todays_date='$causelist_date'  UNION SELECT adjcode, srno,todays_date,order_remark,next_date FROM daily_proc_a where cino='".$link_cino."' and todays_date='$causelist_date' ORDER BY todays_date DESC");
					...
					{
						$link_disposal_qry = $HCMAS_DB->query("SELECT order_remark FROM disposal_proc where cino='".$link_cino."' and todays_date='$causelist_date'  ORDER BY todays_date DESC");
							if($link_disposal_qry->rowcount()>0)
						  {
							  $link_disposal_row = $link_disposal_qry->fetch();
							   
						$link_dis_pos_case=$HCMAS_DB->prepare("SELECT disp_nature,date_of_decision FROM civil_t_a  where cino=:cino and date_of_decision='$causelist_date'  ORDER BY date_of_decision DESC");
						...
						$link_todays_date='';	
						  ...
						  {
							   $link_civil_t_query1 = $HCMAS_DB->query("SELECT disp_nature,date_of_decision FROM civil_t_a where cino='".$link_cino."' and date_of_decision='$causelist_date'  ORDER BY date_of_decision DESC");
						  ...
						  {
							  $link_restore_qry=$HCMAS_DB->prepare("SELECT remark,next_date,todays_date FROM restorerevoke  where cino=:cino  AND datetype='rev' and todays_date='$causelist_date'  ORDER BY todays_date DESC");
			...
			else
			{
                ...
                <td align="left">'.$section.'</td>
                <td align="left">'.$purpose_name.'</td>
									<td align="left">AND</br>'.$link_case_details.'</td>
									<td align="left">'.$link_party_details.'</td>
									<td align="left">'.$link_pet_advocate_details.'</td>
									<td align="left">'.$link_res_advocate_details.'</td>
									<td align="left">'.$link_proceding.'</br>'.$link_proceding_dt.'</td>
									</tr>';
			}
			}
				...
				if($cause_list_row['on_filing']=='Y')
				{
					...
					$civil_t_a_qry = $HCMAS_DB->query("SELECT date_filing_disp,disp_nature FROM civil_t_a where cino='".$cino."' and date_filing_disp='$causelist_date' ORDER BY date_filing_disp DESC");
						 ...
						 $todays_date='';
						  ...
						  {
							  $resSrCaseType = $HCMAS_DB->query("SELECT todays_date,order_remark,next_date FROM daily_proc_filing where cino='".$link_cino."' and todays_date='$causelist_date' ORDER BY todays_date DESC");
				...
				else
				{
					...
					//echo "SELECT adjcode, srno FROM daily_proc where cino='".$cino."' and todays_date='".$causelist_date."' UNION SELECT adjcode, srno FROM daily_proc_a where cino='".$cino."' and todays_date='".$causelist_date."' ORDER BY srno DESC";
					 $daily_pro = $HCMAS_DB->query("SELECT adjcode, srno,todays_date,order_remark,next_date FROM daily_proc where cino='".$cino."' and todays_date='$causelist_date'  UNION SELECT adjcode, srno,todays_date,order_remark,next_date FROM daily_proc_a where cino='".$cino."' and todays_date='$causelist_date'  ORDER BY todays_date DESC");
					...
					{
						$disposal_qry = $HCMAS_DB->query("SELECT order_remark FROM disposal_proc where cino='".$cino."' and todays_date='$causelist_date'  ORDER BY todays_date DESC");
							if($disposal_qry->rowcount()>0)
						  {
							  $disposal_row = $disposal_qry->fetch();
							   
						$dis_pos_case=$HCMAS_DB->prepare("SELECT disp_nature,date_of_decision FROM civil_t_a  where cino=:cino and date_of_decision='$causelist_date'  ORDER BY date_of_decision DESC");
						...
						$todays_date='';	
						  ...
						  {
							   $civil_t_query1 = $HCMAS_DB->query("SELECT disp_nature,date_of_decision FROM civil_t_a where cino='".$cino."' and date_of_decision='$causelist_date'  ORDER BY date_of_decision DESC");
						  ...
						  {
							  $restore_qry=$HCMAS_DB->prepare("SELECT remark,next_date,todays_date FROM restorerevoke  where cino=:cino  AND datetype='rev' and todays_date='$causelist_date'  ORDER BY todays_date DESC");
			...
			else
			{
				$res_cont="";
					...
					$main_cause='<tr style="font-weight:500">
									...
									<td align="left">'.$section.'</td>
									<td align="left">'.$purpose_name.'</td>
									<td align="left">'.$case_details.'</td>
									<td align="left">'.$party_details.'</td>
									<td align="left">'.$pet_advocate_details.'</td>
									<td align="left">'.$res_advocate_details.'</td>
									<td align="left">'.$proceding.'</br>'.$proceding_dt.'</td>
									</tr>';
