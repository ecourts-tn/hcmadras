...
<section id="team" class="pb-5">
    <div class="container">
        <h5 class="section-title h1"><?php echo date("d/m/Y");?> Madurai Bench of Madras High Court Display board <span  id='ct6'></span> <input class="btn btn-default" style="
    background-color: #d00707;
    font-size: 20px;
    color: white;
    font-weight: bold;cursor: pointer;" type="button" value="Refresh" onclick="window.location.href='display_board_mdu.php'"></h5>
...
//echo "SELECT * FROM currentcourt where cur_date='".$Crr_dt."'  order by court asc";
   $qry_display = $DISPLAY_con_MDU->query("SELECT * FROM currentcourt where cur_date='".$Crr_dt."'  order by court asc");
...
while ($row_display = $qry_display->fetch())
	{
	  ...
	  {
		  //var_dump("SELECT casetype,caseno,year,bench_id FROM causelist where cause_date='".$row_display['cur_date']."' and court_no='".$row_display['court']."' and item_no='".$row_display['item']."' and list_no='".$row_display['list']."'");
		  
	   $sql4=$DISPLAY_con_MDU->query("SELECT casetype,caseno,year,bench_id FROM causelist where cause_date='".$row_display['cur_date']."' and court_no='".$row_display['court']."' and item_no='".$row_display['item']."' and list_no='".$row_display['list']."'");
	...
	$cur_dt=date('Y-m-d');
					...
					/* $vc_link_qry=$VC_con->query("SELECT meeting_link FROM tn_vc_det WHERE bench_id = '".$bench_id ."' and court_det='MDU' AND to_date >= '".$cur_dt."' LIMIT 1");
	...
	$case='<a href="#" style="padding:10px" class="btn btn-primary btn-sm">'.$case_det.'</a>';
	  ...
	  else
	  {
		   $case='';
			 ...
			 $sql4=$DISPLAY_con_MDU->query("SELECT casetype,caseno,year,bench_id FROM causelist where cause_date='".$row_display['cur_date']."' and court_no='".$row_display['court']."'  ");
          ...
          <div class="col-xs-12 col-sm-6 col-md-3">
                <div class="" >
                    <div class="mainflip flip-0">
                        <div class="frontside">
                            <div style="border: none;background: #ffffff;">
