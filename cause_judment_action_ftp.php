...
if(isset($_POST['RegCase_type']) and $_POST['RegCase_type']!='' and is_numeric($_POST['RegCase_type']))
{
	...
	if($valid1){  
		...
		$query_main = $HCMAS_DB->query("SELECT * FROM civil_t WHERE regcase_type = '".$case_type."' AND reg_no = '".$case_num."' AND reg_year = '".$case_year."' union SELECT * FROM civil_t_a WHERE regcase_type = '".$case_type."' AND reg_no = '".$case_num."' AND reg_year = '".$case_year."' ORDER BY reg_no ASC ");
...
if(isset($_POST['judge_name']) and $_POST['judge_name']!='')
{
	...
	if($valid3){  
		...
		$query_main = $HCMAS_DB->query("SELECT * FROM judge_name_t WHERE judge_name ilike '%".$judge_name."%' and display='Y'");
...
<?php
		...
		$order_qry=$cond;
		$order_qry_exe =$HCMAS_DB->query($order_qry);
		...
		while($order_row = $order_qry_exe->fetch())
		{
			...
			$case_det_qry = $HCMAS_DB->query("SELECT * FROM civil_t WHERE cino = '".$order_cino."' union SELECT * FROM civil_t_a WHERE cino = '".$order_cino."'");
			 ...
			 $case_details_valu = $reg_case_type.'.'.$case_det_row['fil_no'].'/'.$case_det_row['fil_year'].'<br>(filing stage)';
				...
				else 
				{
					...
					if(($orjc1))
					{
						...
						$query=$HCMAS_DB->query("select * from judge_name_t where judge_code in(".rtrim($order_jocode,",").") and display='Y' order by judge_priority  ");
						...
						$query = $HCMAS_DB->query("select * from judge_name_t where judge_code='".$order_jocode."' and display='Y' "); 
					}
				} //else 	
				...
				<tr class="<?php echo $file_name;?>" id="<?php echo $res_cino;?>">
				   ...
				   <?php if($valid1!=1){ ?>
				    ...
				    <form method="POST" id="<?php echo 'pdf_capt_sub_'.$sno; ?>" action="order_view_ftp.php"  target="_blank" >
					<input type="hidden" name='fileName' id="fileName" value="<?php echo base64_encode($file_name);?>"/>
					<input type='button' value=" PDF " class='text-danger pdf_label' onclick='open_overlay("<?php echo $sno;?>")'/>
