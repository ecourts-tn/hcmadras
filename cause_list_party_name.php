...
$cnr_no='';
$c_no='';
$civ_qry=$HCMAS_DB->query("SELECT cino FROM civil_t WHERE pet_name ILIKE  '%".$party_name."%' OR res_name ILIKE '%".$party_name."%' UNION SELECT cino FROM civil_t_a WHERE pet_name ILIKE  '%".$party_name."%' OR res_name ILIKE '%".$party_name."%'"); 
if($civ_qry->rowCount()>0){
 ...
 $jud_bench_qry=$HCMAS_DB->query("SELECT for_bench_id FROM cause_list WHERE cino IN (".$cnr_num.") and causelist_date='".$causelist_date."'  and elimination='N' UNION SELECT for_bench_id FROM cause_list_a WHERE cino IN (".$cnr_num.") and causelist_date='".$causelist_date."'  and elimination='N'group by for_bench_id");
