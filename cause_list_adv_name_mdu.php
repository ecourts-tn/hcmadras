...
$securimage = new Securimage(array('namespace' => 'advname'));
$valid = $securimage->check($_POST['advname_captcha']);

		/*Validate Session and User Captcha Values*/
		if ($valid){ 		
			
if($_POST['submit'] && $_POST['cause_list_dt']!='' && $_POST['adv_name']!='')
{
...
else if(empty($adv_name))
   {
        echo '<br><h3 style="color:red;text-align:center;">Enter your Advocate Name!</h3>';
     
   }
 else if(!empty($adv_name) && $validator->chkbadchar($adv_name) == false)
 {
	  echo '<br><h3 style="color:red;text-align:center;">Please enter valid Advocate Name</h3>';
...
//$adv_code = $getdata->getAdvCd($bar_no);
 ...
 //"SELECT cino FROM civil_t WHERE pet_adv ILIKE  '%".$adv_name."%' OR res_adv ILIKE '%".$adv_name."%' UNION SELECT cino FROM civil_t_a WHERE pet_adv ILIKE  '%".$adv_name."%' OR res_adv ILIKE '%".$adv_name."%' limit 1500";
//echo "SELECT cino FROM civil_t WHERE pet_adv ILIKE  '%".$adv_name."%' OR res_adv ILIKE '%".$adv_name."%' UNION SELECT cino FROM civil_t_a WHERE pet_adv ILIKE  '%".$adv_name."%' OR res_adv ILIKE '%".$adv_name."%' limit 10";
$cnr_no='';
$civ_qry=$MDU_HCMAS_DB->query("SELECT cino FROM civil_t WHERE pet_adv ILIKE  '%".$adv_name."%' OR res_adv ILIKE '%".$adv_name."%' UNION SELECT cino FROM civil_t_a WHERE pet_adv ILIKE  '%".$adv_name."%' OR res_adv ILIKE '%".$adv_name."%' union

SELECT cino FROM extra_adv_t WHERE adv_name ILIKE '%".$adv_name."%' ");
if($civ_qry->rowCount() > 0)
{
 ...
 // echo "SELECT for_bench_id FROM cause_list WHERE cino IN (".$cnr_num.") and causelist_date='".$causelist_date."'  and elimination='N' group by for_bench_id";
  
 $jud_bench_qry=$MDU_HCMAS_DB->query("SELECT for_bench_id FROM cause_list WHERE cino IN (".$cnr_num.") and causelist_date='".$causelist_date."'  and elimination='N' group by for_bench_id UNION SELECT for_bench_id FROM cause_list_a WHERE cino IN (".$cnr_num.") and causelist_date='".$causelist_date."'  and elimination='N' group by for_bench_id");
...
?>
				...
				<h6 class="post-title" style='color:red;font-family: "Times New Roman", Times, red serif;' align="center">SEARCH RESULTS FOR ADVOCATE NAME - <?php echo $adv_name;?></h6>
...
<table id="advname" class="display responsive nowrap" style="width:100%">
