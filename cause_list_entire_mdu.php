...
$securimage = new Securimage(array('namespace' => 'entirecause'));
$valid = $securimage->check($_POST['entirecause_captcha']);
...
if($_POST['submit'] && $_POST['cause_list_dt']!='')
{
	
if(empty($causelist_date))
   {
      echo 'Enter your Cause List Date !';
   
   }
 else if(!empty($causelist_date) && $validator->chkbadchar($causelist_date) == false)
 {
	  echo 'Please enter valid Cause List Date';
 ...
 else
 {
	 ...
	 $bench_id='';
 $cause_list_title=$MDU_HCMAS_DB->query("SELECT causelist_sr_no,for_bench_id,causelist_id,causelist_date,court_id FROM causelist_title WHERE cause_list_status='P' AND  causelist_date ='".$causelist_date."' UNION SELECT causelist_sr_no,for_bench_id,causelist_id,causelist_date,court_id FROM causelist_title WHERE cause_list_status='P' AND  causelist_date ='".$causelist_date."' order by court_id,causelist_id ");
...
<table id="entire" class="display responsive nowrap" style="width:100%">
