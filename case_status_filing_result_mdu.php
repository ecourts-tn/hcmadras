...
function ChkFileExists($ord_yr,$ord_caseno,$ord_no) {
	...
	curl_close($ch);
	$fil_nme = $ord_yr.'_'.$ord_caseno.'_'.$ord_no.'_MDUfil';
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		if( isset($_POST['filing_captcha']) && !empty($_POST['filing_captcha']) ){
			...
			if($case_name  && $case_no && $case_year){
				...
				$query_main = $MDU_HCMAS_DB->prepare("SELECT * FROM civil_t WHERE filcase_type = :case_name AND fil_no = :caseno AND fil_year = :case_year union SELECT * FROM civil_t_a WHERE filcase_type = :case_name AND fil_no = :caseno AND fil_year = :case_year  ORDER BY reg_no ASC ");
...
<?php					
						...
						if(isset($query_main_result['hide_res_name'])||isset($query_main_result['hide_partyname'])){
							...
							if(trim($query_main_result['hide_res_name'])=='Y' || trim($query_main_result['hide_partyname'])=='Y'){
								...
								$res_det = check_tags(strtoupper(trim($query_main_result['res_name']))).'<br>'.check_tags(trim($resp_address)).' '.$resp_disct_nm.'<br><br>' ; 
						}
						else
							$res_det =  check_tags(strtoupper(trim($query_main_result['res_name']))).'<br>'.check_tags(trim($resp_address)).' '.$resp_disct_nm.'<br><br>' ; 
...
<?php
						   $si=1;
							...
							echo "<tr>";
							echo "<th class='hidden-lg hidden-md'><td class=''>".$si."</td>";
...
<?php
		...
		if($case_name  and $case_no and $case_year){
			...
			$noticess_qry = $MDU_CIS_DB->prepare("SELECT * FROM tn_notice_servestatus where regcase_type = :casenam_e and reg_no = :case_no_o and reg_year = :case_ye_ar  and casenotype = 'F' and despatch_entry_date IS NOT NULL  and display='Y'  order by party_no,type,process_id");
