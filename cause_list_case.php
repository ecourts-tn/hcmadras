			...
			while($link_cause_list_row=$link_cause_list_qry->fetch())	
			{
				...
				if($link_cause_list_row['on_filing']=='Y')
				{
					...
					$link_case_details="SR No. ".$link_fil_type.'.'.$link_RegCase_no.'/'.$link_RegCase_year;
					 ...
					 $link_civil_t_a_qry = $HCMAS_DB->query("SELECT date_filing_disp,disp_nature FROM civil_t_a where cino='".$link_cino."' and date_filing_disp IS NOT NULL  ORDER BY date_filing_disp DESC");
						 ...
						 $link_todays_date=date('d-M-Y',strtotime($disp_c_nat['date_filing_disp']));
						  ...
						  {
							  $link_resSrCaseType = $HCMAS_DB->query("SELECT todays_date,order_remark,next_date FROM daily_proc_filing where cino='".$link_cino."' and todays_date IS NOT NULL  ORDER BY todays_date DESC");
				...
				else
				{
					...
					$link_daily_pro = $HCMAS_DB->query("SELECT adjcode, srno,todays_date,order_remark,next_date FROM daily_proc where cino='".$link_cino."' and todays_date IS NOT NULL  UNION SELECT adjcode, srno,todays_date,order_remark,next_date FROM daily_proc_a where cino='".$link_cino."' and todays_date IS NOT NULL  ORDER BY todays_date DESC");
					...
					{
						$link_disposal_qry = $HCMAS_DB->query("SELECT order_remark FROM disposal_proc where cino='".$link_cino."' and todays_date IS NOT NULL  ORDER BY todays_date DESC");
							if($link_disposal_qry->rowcount()>0)
						  {
							  $link_disposal_row = $link_disposal_qry->fetch();
							   
						$link_dis_pos_case=$HCMAS_DB->prepare("SELECT disp_nature,date_of_decision FROM civil_t_a  where cino=:cino and date_of_decision IS NOT NULL  ORDER BY date_of_decision DESC");
						...
						$link_todays_date='';	
						  ...
						  {
							   $link_civil_t_query1 = $HCMAS_DB->query("SELECT disp_nature,date_of_decision FROM civil_t_a where cino='".$link_cino."' and date_of_decision IS NOT NULL  ORDER BY date_of_decision DESC");
						  ...
						  {
							  $link_restore_qry=$HCMAS_DB->prepare("SELECT remark,next_date,todays_date FROM restorerevoke  where cino=:cino  AND datetype='rev' and todays_date IS NOT NULL  ORDER BY todays_date DESC");
